...
- **Internal test placement.** Complex new unexported logic should be tested white-box in `foo_internal_test.go` (`package foo`). `_internal_test.go` means `package foo`; a black-box test belongs in `foo_test.go`. Nothing catches a mismatch for you — golangci-lint's `testpackage` linter only flags a same-package (`package foo`) declaration, and its default `skip-regexp` exempts `_internal_test.go` precisely so white-box files can use it, so a `package foo_test` declaration in an `_internal_test.go` is invisible to the linter. Don't mix `package foo` and `package foo_test` in one file; don't test public symbols white-box. White-box test files cannot import the same package's `mock/` subdirectory (import cycle); use inline stub types implementing the interface directly in the test file.
...
- **Lint build-tagged packages explicitly.** A package behind a build tag (for example, `//go:build e2e` in `cli/e2e/`) is invisible to `golangci-lint run ./<submodule>/...`, so it can accumulate lint debt silently. Add a project-local `extraCheck` leg in `<project>/project.nix` that runs `golangci-lint run --build-tags <tag> ./<path>/`, and keep an untagged companion file so the directory builds without the tag. See `cli/project.nix` for the working check and `cli/e2e/doc.go` for its companion file.
