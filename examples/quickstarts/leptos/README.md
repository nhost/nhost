...
It talks to the shared [quickstarts backend](../backend), the same one the
React, Vue, Svelte and Next.js quickstarts use. That backend seeds a `movies`
table readable by the `public` role, so the movie list loads immediately, even
before signing in.
...
A backend to talk to. The example points at the local quickstarts backend by
default:
...
```sh
# from examples/quickstarts/leptos
trunk serve --open
```
