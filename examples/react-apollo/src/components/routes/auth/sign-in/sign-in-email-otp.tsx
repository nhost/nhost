...
export default function SignInEmailOTP() {
  ...
  const onSubmitEmail = async (values: z.infer<typeof emailFormSchema>) => {
    const { email } = values;
    const response = await nhost.auth.signInOTPEmail({ email });

    if (!!response.body) {
      // toast.error(response.body.error?.message || 'Failed to send OTP. Please try again.')
  ...
  const signInWithOTP = async () => {
    ...
    if (!!response.body) {
      // toast.error(response.body.error?.message || 'Invalid OTP. Please try again.')
