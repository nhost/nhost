...
export default function Layout() {
  ...
  const { isElevated, checkElevation, removeElevation } = useSecurity();

  const handleElevate = async () => {
    try {
      await checkElevation();
      toast.success("Permissions elevated successfully");
    } catch (error) {
      console.error(error);
      toast.error(`Failed to elevate permissions: ${error}`);
    }
  };

  const handleRemoveElevation = async () => {
    try {
      await removeElevation();
      toast.success("Elevated permissions removed successfully");
    } catch (error) {
      console.error(error);
      toast.error(`Failed to remove elevated permissions: ${error}`);
    }
  };
        ...
        <nav className="flex flex-col items-center gap-4 px-2 mt-auto sm:py-5">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" side="right">
              <DropdownMenuLabel>
                {user?.email} (elevated: {String(isElevated)})
              </DropdownMenuLabel>
              <DropdownMenuSeparator />
              {isElevated ? (
                <DropdownMenuItem onSelect={handleRemoveElevation}>
                  Remove elevation
                </DropdownMenuItem>
              ) : (
