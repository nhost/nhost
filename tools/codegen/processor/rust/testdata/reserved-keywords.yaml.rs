...
impl MatchParams {
    ...
    fn to_headers(&self) -> Vec<(String, String)> {
        ...
        if let Some(v) = &self.r#async {
            headers.push(("async".to_string(), v.to_string()));
        }
        headers
    }
}
...
impl Client {
    ...
    /// Performs POST /items/{r#loop}.
    pub async fn r#match(
        &self,
        r#loop: &str,
        body: KeywordBody,
        params: Option<MatchParams>,
    ) -> Result<Response<KeywordBody>, Error> {
        let url = http::append_path(self.base_url.as_str(), &["items", r#loop])?;
