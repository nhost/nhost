...
pub struct OptionalJsonBody {
    pub message: String,
}


#[derive(Clone, Serialize, Deserialize)]
pub struct OptionalFormBody {
    pub token: String,
}

impl std::fmt::Debug for OptionalFormBody {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let mut debug = f.debug_struct("OptionalFormBody");
...
impl Client {
    ...
    /// Performs POST /json.
    pub async fn optional_json_body(
        &self,
        body: Option<OptionalJsonBody>,
    ) -> Result<Response<()>, Error> {
        let url = http::append_path(self.base_url.as_str(), &["json"])?;
    ...
    /// Performs POST /form.
    pub async fn optional_form_body(
        &self,
        body: Option<OptionalFormBody>,
    ) -> Result<Response<()>, Error> {
        let url = http::append_path(self.base_url.as_str(), &["form"])?;
        let mut request = self.http.request(reqwest::Method::POST, url);
        if let Some(body) = &body {
