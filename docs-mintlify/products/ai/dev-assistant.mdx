---
title: "Developer Assistant"
description: "Empower your developers to code faster and better"
keywords: ["developer assistant", "AI coding", "Graphite", "schema assistant", "developer tools", "ChatGPT"]
icon: hat-wizard
---

The developer assistant is a special AI Assistant enabled through the dashboard that has access to your project's schema and can help developing your project.

To use it simply [enable graphite](/products/ai/enabling-service) and query it through the dasbhoard:

![dashboard](/images/ai/dev-assistants/dashboard.png)

![chat](/images/ai/dev-assistants/chat.png)

## Permissions

Only admins are allowed to interact with the developer assistant.
