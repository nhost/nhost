...
-- identity_check_logs: locks the IsIdentity introspection signal for the
-- TestIntrospect golden. `id` is GENERATED BY DEFAULT AS IDENTITY, which
-- surfaces only via pg_attribute.attidentity — the populateTableColumns
-- SELECT now exposes that bit as is_identity and propagates it to
-- introspection.Column.IsIdentity. Without a real identity column in the
-- fixture the golden silently coalesces every IsIdentity value to false and
-- a regression in the SELECT clause would slip past unnoticed.
