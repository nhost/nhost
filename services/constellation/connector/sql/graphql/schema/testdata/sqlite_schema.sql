...
-- regression. The Postgres twin is GENERATED BY DEFAULT AS IDENTITY; SQLite's
-- closest analogue is INTEGER PRIMARY KEY (rowid alias), which the introspector
-- now flags as IsIdentity for the same reason — the engine populates the value
-- at INSERT time, so the insert-check predicate must run after the row exists.
CREATE TABLE identity_check_logs (
