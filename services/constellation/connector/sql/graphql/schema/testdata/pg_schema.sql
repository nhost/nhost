...
-- identity_check_logs: IDENTITY-column fixture for the requiresPostInsertCheck
-- regression on Postgres GENERATED BY DEFAULT AS IDENTITY primary keys. The id
-- column surfaces only via pg_attribute.attidentity (not pg_attrdef), so
-- IsGenerated and HasDefault both stay false; the IsIdentity bit is what tells
-- the schema generator the engine populates the value at INSERT time so an
-- insert-check predicate referencing it must run after the row exists. The
-- SQLite twin in sqlite_schema.sql uses INTEGER PRIMARY KEY (rowid alias),
-- which the introspector flags as IsIdentity for the same reason.
