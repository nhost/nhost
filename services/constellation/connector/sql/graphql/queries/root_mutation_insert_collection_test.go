...
func TestInsertBuildQuery(t *testing.T) { //nolint:paralleltest,maintidx
	cases := []buildQueryTestCase{
		...
		// AS IDENTITY primary key. Locks the multi-row sibling of the
		// insert_one identity-column case: the post-check path must fire for
		// the batch as a whole so the (id > 0) clause runs against each
		// engine-assigned row in RETURNING rather than the NULL placeholders a
		// pre-check data CTE would carry.
		{
