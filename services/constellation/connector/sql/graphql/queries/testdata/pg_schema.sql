...
INSERT INTO storage.buckets (id) VALUES ('default'), ('profile_pics');
...
-- identity_check_logs: IDENTITY-column + insert-check fixture for the
-- requiresPostInsertCheck regression on Postgres GENERATED BY DEFAULT AS
-- IDENTITY primary keys. The id column surfaces only via pg_attribute.attidentity
-- (not pg_attrdef), so IsGenerated and HasDefault both stay false; without the
-- IsIdentity bit the (id > 0) branch of the insert check would compare against
-- the NULL placeholder a pre-mutation data CTE carries and deny the row. See
-- public_identity_check_logs.yaml for the user role check/preset shape.
