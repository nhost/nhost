...
func TestBuildMutationUpdateByPkSQL(t *testing.T) { //nolint:paralleltest
	cases := []buildQueryTestCase{
		...
		// update_by_pk against a table whose update permission has a non-empty
		// "check" (users: check {id _eq x-hasura-user-id}). Confirms the
		// post-update check CTE is emitted on the by_pk path.
		{
			name: "permissions: update_by_pk own user with check (allowed)",
			query: query{
				Query: `mutation {
					updateUser(
