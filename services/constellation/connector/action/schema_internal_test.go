...
func TestAsyncResponseOutputFieldIsNullable(t *testing.T) {
	...
	conn := newConnector(
		...
		testMetadata(
			...
			customTypes(withObjects(objectType("AsyncOut", nil, objectField("message", "String!")))),
		),
		collector,
	...
	if output.NonNull {
		t.Errorf("async result output field is non-null; want nullable for declared %q", "AsyncOut!")
	}
