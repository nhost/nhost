...
func getSessionVariableFunc(sessionVars map[string]any) jsontmpl.Func {
	return func(arg jsontext.Value) (jsontext.Value, error) {
		...
		case string:
			...
			if !ok {
				return nil, fmt.Errorf("Session variable %q not found", typed) //nolint:staticcheck,stylecheck
			}

			return json.Marshal(value, json.Deterministic(true))
		default:
			return nil, errors.New("Session variable name should be a string") //nolint:staticcheck,stylecheck
		}
	}
}
