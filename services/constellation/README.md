...
- **Wrappers**: `bulk` / `bulk_keep_going` (full native child op set incl. reads, whole-metadata ops, and nested bulk — a nested `bulk` contributes a nested array, a nested `bulk_atomic` an all-or-nothing single object; one composed write per request — a single `resource_version` bump; bare top-level array response), `bulk_atomic` (Hasura's narrow relationship whitelist, all-or-nothing, single `{"message":"success"}` response). See `KNOWN_DIFFERENCES.md` for the exact semantics.
...
What constellation has **no** native handler for falls through to the Hasura upstream when `--hasura-upstream-url` is configured (else returns `not-supported`). This includes, among others: actions, custom types, cron/scheduled triggers, source admin (`pg_add_source` / `pg_drop_source` / `pg_update_source`; sources are config-driven), computed fields, inherited roles, and Apollo Federation. `hasura-cli` migration workflows are out of scope.
