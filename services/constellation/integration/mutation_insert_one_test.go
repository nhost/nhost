...
func TestInsertOneMutations(t *testing.T) { //nolint:paralleltest,maintidx
	cases := []TestCase{
		...
		// behind an orthogonal serialization bug. The parent-field diff still
		// proves the INSERT chain succeeded against the composite FK; the
		// SELECT-side `sets` shape is covered by query tests.
		{
