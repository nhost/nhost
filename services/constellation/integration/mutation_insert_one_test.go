...
type exerciseLogSetRow struct {
	ParentKind string
	Reps       int32
}

func assertCompositeFKNestedArrayInsertSideEffects(t *testing.T) {
	t.Helper()
	...
	rows, err := conn.Query(t.Context(), `
		SELECT parent_kind, reps
		FROM public.exercise_log_sets
		ORDER BY reps
	`)
	if err != nil {
		t.Fatalf("failed to query exercise_log_sets: %v", err)
	}
	defer rows.Close()

	got := []exerciseLogSetRow{}
	for rows.Next() {
		var row exerciseLogSetRow
		if err := rows.Scan(&row.ParentKind, &row.Reps); err != nil {
			t.Fatalf("failed to scan exercise_log_sets row: %v", err)
		}
	...
	if err := rows.Err(); err != nil {
		t.Fatalf("failed to iterate exercise_log_sets rows: %v", err)
	}

	want := []exerciseLogSetRow{
		{ParentKind: "strength", Reps: 3},
		{ParentKind: "strength", Reps: 5},
	}
	if diff := cmp.Diff(want, got); diff != "" {
		t.Fatalf("exercise_log_sets rows mismatch (-want +got):\n%s", diff)
	}
}
...
func TestInsertOneMutations(t *testing.T) { //nolint:paralleltest,maintidx
	cases := []TestCase{
		...
		// Nested array-relationship insert across a *composite* FK. The child
		// table (exercise_log_sets) references the parent (exercise_logs) via
		// FOREIGN KEY (parent_id, parent_kind) → (id, kind). The nested-insert
		// emitter must source each FK column from the matching parent CTE
		// column — parent_kind = mutation_result."kind", not "id". Pre-fix,
		// every nested FK column hardcoded the parent CTE column to "id", so
		// parent_kind would be projected as a uuid into a text column and
		// Postgres would fail at execution with `operator does not exist:
		// uuid = text`, taking the whole CTE chain (including the parent
		// INSERT) down with it. Returning only parent fields here because
		// Constellation's mutation response builder has a separate
		// pre-existing gap that returns nested array-rel children as a single
		// object instead of a list — diffing that here would mask the SQL fix
		// behind an orthogonal serialization bug. The response diff still
		// proves the parent INSERT succeeded; assertConstellationState below
		// separately verifies the nested exercise_log_sets rows.
		{
			name: "composite-FK nested array insert routes each FK column to its matching parent column",
			query: query{
				Query: `
					mutation {
			...
			assertConstellationState: assertCompositeFKNestedArrayInsertSideEffects,
		},
