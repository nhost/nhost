...
func TestInsertMutations(t *testing.T) { //nolint:paralleltest,maintidx
	cases := []TestCase{
		...
		// IDENTITY primary key. The (id > 0) branch can only be evaluated
		// post-INSERT because the engine assigns id at INSERT time; without
		// IsIdentity routing the batch through the post-check path the
		// pre-mutation data CTE would compare NULL against the predicate and
		// every row would be denied.
		{
