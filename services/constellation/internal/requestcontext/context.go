// Package requestcontext stores request-scoped values in a [context.Context],
// with awareness of [*gin.Context] so values stashed on the underlying
// [*http.Request] context are still retrievable when callers pass the gin
// context directly. The stored values are: the inbound HTTP client headers, a
// response-header collector used to forward action webhook response headers
// (e.g. Set-Cookie) back to the caller, and the original GraphQL query string.
// Context keys are unexported, so external callers cannot install a wrong-typed
// value under these keys; a failed type assertion therefore means "not present"
// and is surfaced as a nil/zero value.
//
