...
- **server.js**: Discovers functions via glob, then creates a single shared esbuild context with one entry point per function (shares the parsed dependency graph across functions to keep memory bounded). Change detection uses `chokidar` with `usePolling: true` (inotify/FSEvents don't propagate over Docker bind-mount volumes): `add`/`unlink` of an entry-qualifying file recreates the esbuild context, `change` (or `add`/`unlink` of a non-entry file like one under `_utils/`) triggers an incremental rebuild on the existing context. A second chokidar watcher on `package.json` / lockfiles re-runs `nci` and rebuilds all entries on change. Routes are derived from file paths (e.g., `hello.ts` -> `/hello`). Index files map to parent directories. Builds are output to `/tmp/nhost-build/dist/<safeName>.js`, with generated wrappers in `/tmp/nhost-build/wrappers/`.
