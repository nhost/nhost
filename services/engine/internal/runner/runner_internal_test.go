...
func TestSuperviseTimesOutStuckTierAndCancelsLaterTiers(t *testing.T) {
	...
	stuck := func(context.Context) error { //nolint:unparam // signature must match Service
		close(stuckStarted)
		<-releaseStuck

		return nil
	}
	laterTier := func(ctx context.Context) error { //nolint:unparam // signature must match Service
		<-ctx.Done()
	...
	go func() {
		done <- Supervise(ctx, 20*time.Millisecond, []Service{stuck}, []Service{laterTier})
	}()
