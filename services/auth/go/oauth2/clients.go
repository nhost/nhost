...
func ClientToCreateResponse(
	c sql.AuthOauth2Client, clientSecret *string,
) api.OAuth2CreateClientResponse {
	accessTokenLifetime := int(c.AccessTokenLifetime)
	refreshTokenLifetime := int(c.RefreshTokenLifetime)

	resp := api.OAuth2CreateClientResponse{ //nolint:exhaustruct
		ClientId:             c.ClientID,
		ClientName:           c.ClientName,
