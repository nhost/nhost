...
#   Same test modules as Plan 1, but clients are registered dynamically via
#   POST /oauth2/register (RFC 7591) instead of being pre-configured.
#
...
PLANS=(
    ...
    "oidcc-basic-certification-test-plan[server_metadata=discovery][client_registration=dynamic_client] ${SCRIPT_DIR}/test-config-dcr.json"
    "oidcc-config-certification-test-plan[server_metadata=discovery][client_registration=static_client] ${SCRIPT_DIR}/test-config.json"
)
