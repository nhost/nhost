...
We run three test plans from the conformance suite. Together they validate the core Authorization Code flow, discovery metadata, JWKS, token handling, UserInfo, scopes, client authentication methods, and dynamic client registration.
...
### Plan 2: Basic Certification — Dynamic Client Registration

```
oidcc-basic-certification-test-plan[server_metadata=discovery][client_registration=dynamic_client]
```

Runs the **same test modules** as Plan 1, but instead of using pre-registered clients, the conformance suite dynamically registers its own clients via `POST /oauth2/register` (RFC 7591) before each test. This validates that:

- The registration endpoint is discoverable via `/.well-known/openid-configuration`
- Dynamically registered clients work correctly through the full flow
- The server properly stores and uses client metadata from registration

**Additional RFC coverage:** RFC 7591 (Dynamic Client Registration)

### Plan 3: Config Certification — Discovery & Metadata
...
4. Create the demo user and 3 OAuth2 clients
5. Write `test-config.json` and `test-config-dcr.json`
...
This runs all three test plans sequentially and reports pass/fail for each.
...
# Basic Certification (dynamic client registration)
CONFORMANCE_SERVER=https://localhost.emobix.co.uk:8443 \
CONFORMANCE_DEV_MODE=1 \
DISABLE_SSL_VERIFY=1 \
python3 scripts/run-test-plan.py \
    'oidcc-basic-certification-test-plan[server_metadata=discovery][client_registration=dynamic_client]' \
    ../test-config-dcr.json
...
7. Paste the contents of `test-config.json` (or `test-config-dcr.json` for dynamic)
