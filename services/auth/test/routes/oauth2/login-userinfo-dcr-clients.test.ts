...
describe('login-userinfo-dcr-clients', () => {
  ...
  it('should reject DCR registration without authentication', async () => {
    try {
      await nhost.auth.oauth2Register({
        client_name: 'Unauthorized DCR Client',
        redirect_uris: [REDIRECT_URI],
      });
      expect(true).toBe(false);
    } catch (err) {
  ...
  it('should reject DCR with empty redirect_uris', async () => {
    try {
      await nhost.auth.oauth2Register(
        {
          client_name: 'Empty Redirect URIs Client',
          redirect_uris: [],
        },
        { headers: { Authorization: `Bearer ${jwt}` } },
      );
