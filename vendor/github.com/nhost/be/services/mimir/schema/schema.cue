...
#Config: {
	...
	if experimental.nhost != _|_ {
		...
		_validateResourcesTotalCpuMin1000: _totalResourcesCPU >= 1000 & true @cuegraph(skip)
...
#Experimental: {
	...
	// for custom auth domains. Engine-derived values take precedence over
	// same-named global.environment entries, unlike standalone containers; this
	// prevents generic environment overrides from replacing consolidated secrets,
	// least-privilege database connections, or other authoritative engine config.
	// The engine always runs constellation as its GraphQL engine, so it is mutually
	// exclusive with the standalone experimental.constellation service (enforced
