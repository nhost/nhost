...
You can find details on how to connect to the database from an [Nhost Run](/products/run/overview) service [here](/products/run/networking#connecting-to-the-nhost-stack). If you don't know the password you can set a new password in the dashboard:
...
[Functions](/products/functions/overview) run on a separate network, which means in order to access the database you will first need to [make it public](#public-access).
