...
docker run -it --rm \
  ...
  certbot/dns-route53 certonly -v --dns-route53 --dns-route53-propagation-seconds 60 -d local.nhost.run -d local.graphql.nhost.run -d local.auth.nhost.run -d local.storage.nhost.run -d local.functions.nhost.run -d local.mail.nhost.run -m 'siarhei@nhost.io' --non-interactive --agree-tos --server https://acme-v02.api.letsencrypt.org/directory

cp letsencrypt/live/local.nhost.run/fullchain.pem internal/ssl/.ssl/
cp letsencrypt/live/local.nhost.run/privkey.pem internal/ssl/.ssl/
