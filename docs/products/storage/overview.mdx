...
Public URLs are available for both unauthenticated and authenticated users. Permissions are checked for every file request. To get a public URL for a file you can use the following method:
...
// then you can just use resp.url as a src
```
