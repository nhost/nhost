---
title: Overview
description: Learn about Nhost Auth
icon: hand-wave
---

Nhost Auth is a ready-to-use authentication service seamlessly integrated with the [GraphQL API](/products/graphql) and its [Permission System](/guides/api/permissions) from Hasura. This allows you to easily add user authentication to your application without having to build and maintain your own authentication system.

## Supported Methods

<CardGroup cols={4}>
  <Card title="Email and Password" icon="square-1" href="../guides/auth/sign-in-email-password">
  </Card>
  <Card title="One-Time Passwords (OTP)" icon="square-2" href="../guides/auth/sign-in-otp">
  </Card>
  <Card title="Magic Link" icon="square-3" href="../guides/auth/sign-in-magic-link">
  </Card>
  <Card title="Phone Number (SMS)" icon="square-4" href="../guides/auth/sign-in-phone-number">
  </Card>
  <Card title="Security Keys (WebAuthn)" icon="square-5" href="../guides/auth/sign-in-webauthn">
  </Card>
  <Card title="ID Tokens" icon="square-6" href="../guides/auth/sign-in-idtokens">
  </Card>
  <Card title="Elevated Permissions" icon="square-7" href="../guides/auth/elevated-permissions">
  </Card>
</CardGroup>

### OAuth Providers

<CardGroup cols={4}>
  <Card title="Apple" icon="apple" href="/products/auth/social/sign-in-apple">
  </Card>
  <Card title="Discord" icon="discord" href="/products/auth/social/sign-in-discord">
  </Card>
  <Card title="Facebook" icon="facebook" href="/products/auth/social/sign-in-facebook">
  </Card>
  <Card title="GitHub" icon="github" href="/products/auth/social/sign-in-github">
  </Card>
  <Card title="Google" icon="google" href="/products/auth/social/sign-in-google">
  </Card>
  <Card title="Linkedin" icon="linkedin" href="/products/auth/social/sign-in-linkedin">
  </Card>
  <Card title="Spotify" icon="spotify" href="/products/auth/social/sign-in-spotify">
  </Card>
  <Card title="Twitch" icon="twitch" href="/products/auth/social/sign-in-twitch">
  </Card>
  <Card title="WorkOS" icon="square-9" href="/products/auth/social/sign-in-workos">
  </Card>
</CardGroup>
