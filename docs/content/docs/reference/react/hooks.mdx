...
const { signUp, isLoading, isSuccess, needsVerification, isError, error } =
  useEmailPasswordSignUp(email?: string, password?: string, options?: SignUpOptions )
```

- `signUp: (email?: string, password?: string) => void` triggers the user registration. The email/password arguments will take precedence over the possible state values used when creating the hook.
- `isLoading: boolean` returns `true` when the action is executing, `false` when the action result is available.
- `needsVerification: boolean` returns `true` if the registration has been accepted, but a verificaiton email has been sent and is awaiting.
- `isSuccess: boolean` returns `true` if the registration suceeded. Returns `false` if the new email needs to be verified first, or if an error occurred.
...
- `SignUpOption`:
  ...
  - `displayName?: string`
  - `metadata?: Record<string, unknown>`: Custom additional user information stored in the `metadata` column.
...
const { signIn, isLoading, isSuccess, isError, error } =
  useEmailPasswordlessSignIn(email?: string, options?: PasswordlessOptions)
```
...
When an user lost their password, we can resend them an email to authenticate, so they can change their password:
