...
```js
const { signUp, isLoading, isSuccess, needsEmailVerification, isError, error } =
...
```js
const { signIn, isLoading, needsEmailVerification, needsMfaOtp, sendMfaOtp, isSuccess, isError, error } =
...
const Component = () => {
  const [email, setEmail] = useState('')
  const [password, setPassword] = useState('')
  const { signIn, isLoading, isSuccess, needsEmailVerification, isError, error } =
