...
See the [environment variables](/reference/hasura-auth/environment-variables) for additional information about how to connnec to an SMTP server.
...
- After Oauth providers complete or fail authentication, they redirect to the Hasura Auth callback. Hasura auth then redirects to the frontend
- Every email sent to the user (passwordless with email, password/email change, password reset) contains a link, that will eventually redirect to the frontend
...
The `auth.users.metadata` field is a basic JSON column, that can be used as an option on registration:
