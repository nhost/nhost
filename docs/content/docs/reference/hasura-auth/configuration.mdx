...
By default, anyone can sign up to Hasura Auth. Signup can be deactivated in setting `AUTH_DISABLE_NEW_USERS` to `true`.

It is also possible to activate additional checks.
