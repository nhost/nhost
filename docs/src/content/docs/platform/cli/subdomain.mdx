...
description: Find the local service URLs printed by nhost up and use the local subdomain and region to configure the Nhost SDK.
keywords: ["subdomain", "region", "local URLs", "SDK configuration", "dynamic DNS", "offline access"]
head:
  - tag: title
    content: "Local Service URLs and SDK Connection Settings | Nhost Docs"
---
...
`nhost --local-subdomain 192-168-1-108 up` showing the rewritten service URLs. Replace with `<AsciinemaPlayer src="/casts/cli/local-subdomain.cast" idleTimeLimit={2} />` (add the `AsciinemaPlayer` import at the top of the file).
:::
