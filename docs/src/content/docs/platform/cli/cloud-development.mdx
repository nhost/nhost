...
:::note[📷 Screenshot idea — remove if not needed]
Nhost Cloud Console **Settings → Database** showing the **Public Access** toggle and the **Reset Password** / connection string. Replace with `![Database settings](/images/cli/cloud-database-settings.png)`.
:::
...
:::note[▶️ Asciinema idea — remove if not needed]
`nhost up cloud` connecting to a cloud project and printing the local URLs. Replace with `<AsciinemaPlayer src="/casts/cli/cloud-up.cast" poster="npt:0:05" idleTimeLimit={2} />` (add `import { AsciinemaPlayer } from '@components';` at the top of the file).
:::
