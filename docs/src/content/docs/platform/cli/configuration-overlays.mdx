...
description: Use RFC 6902 overlays to apply per-environment tweaks to a single Nhost config across staging, production, and local development.
keywords: ["configuration overlays", "environment configuration", "RFC6902", "config management", "staging", "production"]
head:
  - tag: title
    content: "Customize Config per Environment with Overlays | Nhost Docs"
---
...
`nhost config edit` → `nhost config show` → `nhost config validate` end-to-end. Replace with `<AsciinemaPlayer src="/casts/cli/config-overlays.cast" idleTimeLimit={2} />` (add the `AsciinemaPlayer` import at the top of the file).
:::
