...
Give your AI assistant the link <CopyText text="https://docs.nhost.io/install-mcp" /> and it will read the onboarding instructions and set up the MCP server for your local project. For a project on Nhost Cloud, use <CopyText text="https://docs.nhost.io/install-mcp-cloud" /> instead.

See [Development Setup](/platform/cli/mcp/development-setup#let-an-ai-assistant-set-it-up) and [Cloud Setup](/platform/cli/mcp/cloud-setup) for the per-client commands and the manual steps.
:::

Not using the CLI at all? Nhost runs a hosted connector for the platform at <CopyText text="https://mcp.nhost.io" /> — see [Nhost Cloud connector](/platform/cloud/mcp). [Agentic workflows](/getting-started/agentic-workflows) compares all three setups.
...
For more details, see the [configuration](/platform/cli/mcp/configuration) page. For what these settings mean once an assistant is pointed at a project holding real data, see [Scoping access](/platform/cli/mcp/cloud-setup#scoping-access).
