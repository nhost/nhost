...
An AI assistant (e.g. Claude Desktop or Cursor) connected to the Nhost MCP server, running a GraphQL query against a project. Replace with `![AI assistant using Nhost MCP](/images/cli/mcp-assistant.png)`.
:::

:::note[▶️ Asciinema idea — remove if not needed]
`nhost mcp config` generating the config, then `nhost mcp start`. Replace with `<AsciinemaPlayer src="/casts/cli/mcp.cast" idleTimeLimit={2} />` (add the `AsciinemaPlayer` import at the top of the file).
:::
