...
The configuration has two kinds of sections. The `[cloud]` section controls access to the Nhost Cloud platform (organizations and project configuration). Each `[[projects]]` block configures one individual project's data and schema. Your local project is just a `[[projects]]` entry with `subdomain = "local"`.
