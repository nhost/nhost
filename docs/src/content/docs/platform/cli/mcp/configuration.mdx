...
keywords: ["MCP configuration", "mcp-nhost.toml", "project configuration", "admin secret", "PAT", "metadata management"]
head:
  - tag: title
    content: "Configure the mcp-nhost.toml Server File | Nhost Docs"
---
...
There are two sections in the configuration file: `[cloud]` and `[[projects]]`.

The `[cloud]` section controls access to the Nhost Cloud platform, such as managing your organizations and project configuration.

The `[[projects]]` section configures a single project's data and schema. Add one block per project you want to access. Your local project is just a `[[projects]]` entry with `subdomain = "local"`.

```toml title="mcp-nhost.toml"
...
# When enabled, the manage-graphql tool can create migrations and manage metadata
...
# Optional: Custom GraphQL engine URL (for metadata management)
...
The syntax is `$VAR_NAME`, without braces. `${VAR_NAME}` is not expanded — it is passed through unchanged and would be used as the literal credential. Escape a literal dollar sign as `$$` or `\$`.
...
| `hasura_url` | string | No | Custom GraphQL engine endpoint URL. Auto-generated if not provided. Required for `manage_metadata`. |
