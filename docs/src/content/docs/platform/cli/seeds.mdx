---
title: Seeds
description: Create reusable seed files with the Nhost CLI to extract data and pre-populate your local database for testing and team development.
keywords: ["database seeds", "test data", "seed files", "development data", "data population"]
head:
  - tag: title
    content: "Populate Your Local Database with Seed Data | Nhost Docs"
---
...
The local dashboard showing the `animals` table populated with rows before extracting the seed. Replace with `![animals table data](/images/cli/seeds-animals-table.png)`.
:::
...
`nhost dev hasura seed create …` then `nhost up --apply-seeds` planting the data. Replace with `<AsciinemaPlayer src="/casts/cli/seeds.cast" idleTimeLimit={2} />` (add the `AsciinemaPlayer` import at the top of the file).
:::
