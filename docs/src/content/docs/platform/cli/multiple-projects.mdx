...
Two projects running in parallel on different ports, then `nhost --project-name … dev logs`. Replace with `<AsciinemaPlayer src="/casts/cli/multiple-projects.cast" idleTimeLimit={2} />` (add the `AsciinemaPlayer` import at the top of the file).
:::
