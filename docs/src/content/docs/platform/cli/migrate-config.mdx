...
The full migration flow — `nhost login` → `nhost link` → `nhost config pull` → `nhost up`. Replace with `<AsciinemaPlayer src="/casts/cli/migrate-config.cast" idleTimeLimit={2} />` (add the `AsciinemaPlayer` import at the top of the file).
:::
