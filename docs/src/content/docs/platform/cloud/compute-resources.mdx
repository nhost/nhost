...
keywords: ["compute resources", "vCPU", "RAM", "dedicated compute", "shared compute", "encryption at rest", "disk IOPS"]
head:
  - tag: title
    content: "Allocate vCPU and RAM to Your Backend | Nhost Docs"
---
...
:::tip
To further improve availability and fault tolerance, check out [Service Replicas](/platform/cloud/service-replicas#service-replicas)
:::
...
When using dedicated compute we allow your application to use more than its allotted CPU resources if those resources are available. This means what we are calling dedicated compute is, in fact, guaranteed compute. For instance:
...
- In scenario A the green application is barely using its allotted CPU so if the other applications need it, they can borrow it.
...
All files uploaded to the [storage](/products/storage) service are encrypted at rest using AES-256 encryption. Similarly, any volumes provisioned for your [database](/products/database) and [Run services](/products/run) are also encrypted using AES-256.
