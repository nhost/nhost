---
title: Rate Limits
description: Throttle GraphQL, Storage, Functions, and Auth requests by client IP to prevent brute-force attacks and API abuse.
keywords: ["rate limits", "API protection", "brute force prevention", "request throttling", "abuse prevention", "IP rate limiting"]
head:
  - tag: title
    content: "Throttle Requests to Prevent API Abuse | Nhost Docs"
sidebar:
  label: Rate Limits
---
...
| Any endpoint that an attacker may try to brute-force. This includes sign-in, verify, and OAuth2 authorize/login endpoints<sup>3</sup> | Client IP | 10 / 5 minutes | Configurable | 0.33.0 |
| Signup endpoints<sup>4</sup> | Client IP | 10 / 5 minutes | Configurable | 0.33.0 |
| OAuth2 server-to-server endpoints<sup>5</sup> | Client IP | 100 / 5 minutes | Configurable. Higher limit to accommodate backend services making requests on behalf of multiple users from a single IP | - |
...
- `/user/email/change`
- `/user/password/reset`
- `/user/email/send-verification-email` - If email verification enabled
