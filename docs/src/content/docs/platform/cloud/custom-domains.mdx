...
description: Map your own branded domains to Auth, GraphQL, Postgres, and Run services using CNAME records via the dashboard or config file.
keywords: ["custom domain", "domain", "DNS", "CNAME", "branding", "white label", "subdomain"]
head:
  - tag: title
    content: "Map Your Own Branded Domain to Nhost Services | Nhost Docs"
---
...
You can configure Custom Domains for Auth, GraphQL, PostgreSQL, and your own Run services using both the Nhost Dashboard or the Config file.
...
<Tabs>
  <TabItem label="Dashboard">
    Open the custom-domain settings for the service you want to configure:

    - **PostgreSQL:** [Database → Settings → Custom Domain](https://app.nhost.io/orgs/_/projects/_/database/settings?tab=custom-domain).
    - **Auth, GraphQL, Functions, and Run:** [Settings → Custom Domains](https://app.nhost.io/orgs/_/projects/_/settings/custom-domains).
  ...
  <TabItem label="Config File">
    ...
    For GraphQL, Auth, and Functions, custom domains are defined in the default `./nhost/config.toml` as follows:
...
:::note
After configuring your custom domains don't forget to update your Nhost client to make use of them. For instance, when using our [SDK](/reference/javascript/nhost-js/main#createclient):
