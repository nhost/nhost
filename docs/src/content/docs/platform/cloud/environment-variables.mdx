...
description: Set runtime environment variables shared across all services, including Run services and Functions, via config or the dashboard.
keywords: ["environment variables", "env vars", "configuration", "settings", "runtime config", "env"]
head:
  - tag: title
    content: "Set Runtime Configuration for Your Services | Nhost Docs"
---
...
```ts title="functions/print-env.ts"
