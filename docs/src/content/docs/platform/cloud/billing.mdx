---
title: Billing and Organizations
description: Understand Nhost organizations, subscription plans, billing cycles, and how to downgrade or transfer projects to a Starter plan.
keywords: ["billing", "pricing", "subscription plans", "organizations", "compute costs", "invoices"]
head:
  - tag: title
    content: "Subscription Plans, Pricing, and Invoices | Nhost Docs"
---
...
Direct downgrades from a paid plan to the Starter (free) plan are not available. To move to Starter:

1. Create a new Starter organization (or use an existing one).
2. Pause the projects you want to keep.
3. Transfer each paused project to the Starter organization via **Project Settings > Transfer**.
4. Once all projects have been moved, cancel the paid organization from the Stripe Customer Portal.

Note that only paused projects can be transferred to a Starter organization and a Starter organization can only have one active project at a time.

Before transferring, be aware that Starter organizations have lower resource limits than paid plans (e.g. postgres disk size, function execution timeout, number of functions per deployment). If your project exceeds the Starter limits you may need to reduce its resources before unpausing it in the new organization. Refer to the [pricing page](https://nhost.io/pricing) for a full comparison of plan limits.
