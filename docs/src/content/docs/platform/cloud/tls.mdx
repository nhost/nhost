...
description: Enable mTLS client authentication on custom domains, requiring CA-signed client certificates and forwarding TLS headers to your service.
keywords: ["TLS", "SSL", "client authentication", "mTLS", "client certificates", "security"]
head:
  - tag: title
    content: "Advanced TLS and mTLS Client Certificate Setup | Nhost Docs"
---
...
- `ssl-client-cert`: The client certificate that was used
...
Now we can proceed to create client certificates. You can repeat the steps below to create as many as you need.
...
$ curl --cert client.crt --key client.key https://functions.acme.com/v1/echo | jq
{
  "headers": {
    "accept": "*/*",
    "ssl-client-cert": "-----BEGIN%20CERTIFICATE-----...omitted for brevity...-----END%20CERTIFICATE-----%0A",
