---
title: Permissions
description: Control file access with the GraphQL permission system
keywords: ["storage", "permissions", "access control", "authorization", "roles", "security"]
head:
  - tag: title
    content: "Control File Access by Role | Nhost Docs"
---
...
A simpler approach uses the file's custom `metadata` JSONB column to store the organization ID, and a [computed field](/products/graphql/computed-fields) or relationship to evaluate it.
...
Storage permissions are built on the GraphQL permission system for the `storage.files` table. The dashboard configures these automatically, but understanding the underlying mapping can help with debugging or advanced setups.
...
3. Storage forwards these as session variables (`X-Hasura-User-Id`, `X-Hasura-Role`, etc.)
4. The GraphQL engine evaluates the permission rules for the `storage.files` table
...
Each dashboard action maps to a permission on the `storage.files` table:

| Action | Permission | Required Columns |
|--------|------------------|-----------------|
