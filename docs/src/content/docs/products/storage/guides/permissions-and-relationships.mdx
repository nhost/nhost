...
keywords: ["storage", "permissions", "relationships", "GraphQL", "communities", "multi-tenant", "guide"]
head:
  - tag: title
    content: "Secure & Link Files with GraphQL Relationships | Nhost Docs"
---
import { Tabs, TabItem } from '@astrojs/starlight/components';


This guide walks through two common file-access patterns: **personal files** (users access only their own) and **community files** (shared across a group via a junction table and membership). Both patterns build on [permissions](/products/graphql/permissions) applied to the `storage.files` table.
...
Because metadata lives in your database, you can query files via GraphQL — permissions are applied automatically:
