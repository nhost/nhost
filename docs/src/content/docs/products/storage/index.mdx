...
[Permissions](/products/storage/permissions) follow a **Zero Trust** model -- no role has access by default. You grant access by configuring `insert`, `select`, and `delete` permissions on the `storage.files` table. The same session variables (`X-Hasura-User-Id`, `X-Hasura-Role`, custom claims) available in your GraphQL permissions also work for storage.
...
<CardGroup cols={3}>
  ...
  <Card title="Permissions" icon="lock" href="/products/storage/permissions">
    Control access with the GraphQL permission system
  </Card>
