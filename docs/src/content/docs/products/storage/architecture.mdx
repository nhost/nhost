...
keywords: ["storage", "architecture", "S3", "GraphQL", "authentication", "integration"]
head:
  - tag: title
    content: "How File Storage Works with S3 & GraphQL | Nhost Docs"
---
...
2. Checks permissions via the GraphQL API
3. Performs the file operation against S3
4. Manages file metadata in the database

### GraphQL API

File metadata is stored in PostgreSQL in the `storage` schema and exposed through the GraphQL API. This gives you:

- **Metadata queries** -- query files and buckets using the full power of GraphQL (filtering, sorting, pagination, relationships)
- **Permission enforcement** -- the same permission system that protects your application data also protects file access
- **Relationships** -- create GraphQL relationships between your application tables and `storage.files`

### Auth Integration

Storage uses the same JWT tokens issued by [Nhost Auth](/products/auth). When a user makes a request to the Storage API, the service extracts the session information (user ID, role, custom claims) from the JWT and forwards it to the GraphQL engine for permission evaluation. No separate authentication is needed.
...
Because file metadata lives in your database, you can query it like any other table:
