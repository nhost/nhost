...
head:
  - tag: title
    content: "Resize & Optimize Images On the Fly | Nhost Docs"
---
...
|-----------|-------------|--------|
| `w` | Maximum width (maintains aspect ratio) | Pixels, `1` -- `8000` |
| `h` | Maximum height (maintains aspect ratio) | Pixels, `1` -- `8000` |
| `q` | Image quality | `1` -- `100` (applies to JPEG, WebP, AVIF, HEIC) |
...
:::note
`w`, `h`, and `b` are bounded to keep a single transformation from exhausting server memory. Values above the maximum are rejected with a `400` error rather than silently clamped, so the caller learns the limit instead of receiving an image at an unexpected size. The values shown are the defaults; self-hosted instances can tune them with the `IMAGE_TRANSFORMER_MAX_DIMENSION` and `IMAGE_TRANSFORMER_MAX_BLUR_SIGMA` environment variables on the storage service.
:::
