---
title: Antivirus
description: Scan every uploaded file for malware with ClamAV before it reaches S3, rejecting infected files automatically.
keywords: ["antivirus", "ClamAV", "malware scanning", "file security", "virus detection", "storage security"]
head:
  - tag: title
    content: "Malware Scanning for File Uploads | Nhost Docs"
---
...
:::tip
The `storage.virus` table can be used as a source for [Event Triggers](/products/events/event-triggers), allowing you to send alerts, block users, or trigger automated workflows when a virus is detected.
:::
...
7. You can verify entries were added to the `virus` table via GraphQL:
