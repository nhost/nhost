...
Nhost Functions are server-side JavaScript or TypeScript functions that are deployed as HTTP endpoints. They are a great option for handling [events](/products/graphql/events/) (event triggers, cron triggers, and one-off scheduled events), integrating with external services like Stripe or Slack, and building custom API logic.
...
<CardGroup cols={3}>
  ...
  <Card title="Event Webhooks" icon="plug" href="/products/graphql/events">
