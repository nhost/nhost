...
description: Deploy JavaScript and TypeScript serverless functions as HTTP API endpoints for webhooks, event triggers, and custom backend logic.
...
head:
  - tag: title
    content: "JavaScript & TypeScript Serverless Functions | Nhost Docs"
---
import { Card, CardGroup } from '@components';

Nhost Functions are server-side JavaScript or TypeScript functions that are deployed as HTTP endpoints. They are a great option for handling [events](/products/events) (event triggers, cron triggers, and one-off scheduled events), integrating with external services like Stripe or Slack, and building custom API logic.
...
<CardGroup cols={3}>
  <Card title="Local Development" icon="laptop-code" href="/products/functions/local-development">
    Run and test functions locally with the Nhost CLI
  </Card>
  ...
  <Card title="Metrics" icon="chart-line" href="/products/functions/metrics">
    Invocations, response times, and error rates (Pro/Team)
  </Card>
...
<CardGroup cols={3}>
  ...
  <Card title="Generating Metrics" icon="chart-line" href="/products/functions/guides/generating-metrics">
    Deploy a sample function to populate the Metrics dashboard
  </Card>
  ...
  <Card title="Event Webhooks" icon="plug" href="/products/events">
    Handle event triggers, cron triggers, and one-off scheduled events
  </Card>
</CardGroup>
