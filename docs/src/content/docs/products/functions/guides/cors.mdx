...
Nhost Functions set a small set of default CORS headers so simple cross-origin `GET` and `POST` requests work without extra code. Other methods, custom headers, or credentials are your responsibility.
...
Headers you set replace the default; headers you don't keep it. The snippet above covers simple requests only — see [Preflight Requests](#preflight-requests) for anything else.
