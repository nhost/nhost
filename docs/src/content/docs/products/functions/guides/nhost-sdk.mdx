...
head:
  - tag: title
    content: "Calling GraphQL, Storage & Auth from Functions | Nhost Docs"
---
...
This pattern is useful when you want to add server-side logic (validation, side effects, external API calls) while still respecting the user's permissions.
...
```ts title="./functions/admin-action.ts"
import type { Request, Response } from 'express'
...
:::caution
Admin requests bypass all permissions. Make sure your function validates the caller's authorization before performing privileged operations.
:::
...
```ts title="./functions/generate-report.ts"
import type { Request, Response } from 'express'
import { createClient, withAdminSession } from '@nhost/nhost-js'
