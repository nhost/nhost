...
The Project MCP server gives any Nhost project an agentic interface: it exposes your GraphQL API as an <a href="https://modelcontextprotocol.io/" target="_blank" rel="noopener noreferrer">MCP</a> server, so assistants like Claude, Cursor, or any MCP-compatible client can query and mutate your data on behalf of your users.
...
- **Open source** — Project MCP is a small Go binary you can read, extend, and self-host. The <a href="https://github.com/nhost/nhost/tree/main/services/mcp" target="_blank" rel="noopener noreferrer">source lives in the monorepo</a>.
...
## Project MCP server vs. CLI MCP server

Nhost offers two MCP servers for different jobs:

| | **Project MCP** | **[CLI MCP](/platform/cli/mcp)** |
|---|---|---|
| Use it to | Ship an AI interface to your app's users | Build and manage your project with AI |
| Runs | Deployed with your project, on Nhost Run | Locally, via `nhost mcp start` |
| Acts as | Each end user, through OAuth2 | You (admin secret or PAT) |
| Tools | `get-schema`, `graphql-query`, `graphql-mutation` | `get-schema`, `graphql-query`, `manage-graphql`, `cloud-graphql-query`, `search` |

:::note
Both share `get-schema` and `graphql-query`, but run as different identities: Project MCP is scoped to the signed-in user's permissions; CLI MCP runs as you (up to `admin`). So *"list every order"* returns only that user's orders through Project MCP, but every order through CLI MCP.
:::
