...
1. **Enable OAuth2 Provider in Settings**

   Project MCP relies on Nhost Auth acting as an OAuth2 authorization server. Enable it and configure an RSA signing key.

   ![oAuth2 Provider](/images/products/ai/mcp/authentication/oauth2-provider.png)

   Learn more: [OAuth2 / OIDC Provider](/products/auth/oauth2-provider)

2. **A Consent Page**

   After enabling OAuth2, you must add a Login URL for a consent page. If you need tips on building a consent page, check out our [consent page guide](/products/auth/oauth2-provider/authorization-flow#what-you-build-the-consent-page).

   ![oAuth2 Provider Configs](/images/products/ai/mcp/authentication/oauth2-provider-configs.png)
...
3. **CIMD enabled**
   ...
   ![CIMD](/images/products/ai/mcp/authentication/cimd.png)
...
```json title="/.well-known/oauth-protected-resource"
{
...
```json title="/.well-known/oauth-authorization-server"
{
