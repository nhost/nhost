---
title: Performance
description: Optimize PostgreSQL performance by tuning configuration, indexing, finding slow queries, and scaling compute and disk resources.
keywords: ["PostgreSQL", "database performance", "slow queries", "pghero", "indexes", "pg_stat_statements", "query optimization"]
head:
  - tag: title
    content: "Optimizing PostgreSQL Database Performance | Nhost Docs"
---
...
[PgHero](https://github.com/ankane/pghero) is one of such tools you can use to idenfity and address slow queries. You can easily run pghero alongside your postgres with [Nhost Run](/products/run):
...
[PgHero](https://github.com/ankane/pghero), in addition to help with slow queries, can also help finding missing and duplicate indexes. See previous section on how to deploy pghero with [Nhost Run](/products/run).
