---
title: Backups
description: Daily backups, 7-day retention, manual pg_dump backups, and point-in-time recovery to protect and restore your PostgreSQL data.
keywords: ["backups", "restore", "recovery", "pg_dump", "point-in-time recovery", "PITR", "data protection", "disaster recovery"]
head:
  - tag: title
    content: "Database Backups and Point-in-Time Recovery | Nhost Docs"
---
...
All Pro and Team projects come with daily backups enabled with a 7-day retention by default. You can view, download and restore your project's backups by visiting the [Backups](https://app.nhost.io/orgs/_/projects/_/database/backups) section in your project's dashboard.

### Restoring a Backup

To restore a backup, go to the [Backups](https://app.nhost.io/orgs/_/projects/_/database/backups) section in your project's dashboard, pick the backup you want to restore from and click on "Restore". Keep in mind this will wipe out all of the project's data prior to initiating the restore.

### Restoring a Backup on a Different Nhost Project

You can restore a daily backup directly onto another Nhost project from the dashboard, without downloading it. The source and target projects must be in the **same organization and region**.

:::caution
Restoring wipes out all of the target project's data prior to initiating the restore.
:::

1. Go to the target project's [Backups](https://app.nhost.io/orgs/_/projects/_/database/backups) section and open the "Import Backup" tab.
2. Select the source project you want to import a backup from. Only projects in the same organization and region are listed.
3. The source project's daily backups are listed. Pick the backup you want and click "Import backup".

This triggers the restore in the background. You can follow the progress in the target project's logs by filtering on "Service: Backup Jobs".

:::note
If the source project has Point-in-Time Recovery enabled, the "Import Backup" tab shows the point-in-time restore flow instead (see [Restoring to a Specific Point in Time on a Different Project](#restoring-to-a-specific-point-in-time-on-a-different-project)).
:::

### Restoring a Backup Locally or to an External Database

You can also restore a backup to a database running locally (or any external PostgreSQL) by downloading it and restoring it manually:
...
1. Go to the [Backups](https://app.nhost.io/orgs/_/projects/_/database/backups) section in your project's dashboard and download the backup you want.
...
<Tabs>
 <TabItem label="Dashboard">
  1. Go to the [database settings](https://app.nhost.io/orgs/_/projects/_/database/settings?tab=point-in-time) section in your project's dashboard.
...
To restore to a specific point in time:
  ...
  Go to [Database → Backups → Point-in-Time](https://app.nhost.io/orgs/_/projects/_/database/backups/point-in-time) in your project's dashboard and click on "Start Restore"
...
Go to Database > Settings in both the new and the original projects and make sure the new project is running the same exact database version as the original project.



 ### Restore backup

Finally, go to the new project's [Backups](https://app.nhost.io/orgs/_/projects/_/database/backups) section, navigate to "Import Backup" and select the original project. After this the steps are the same as restoring to a specific point in time on the same project.
