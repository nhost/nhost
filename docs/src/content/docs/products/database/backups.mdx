...
:::caution
Because pg_durable does not register its workflow tables for logical dumps, daily backups and manual `pg_dump` backups do not include pg_durable workflow state. Restoring or importing one loses workflows and can leave orphaned engine data behind. This is a limitation of the [pg_durable extension](/products/database/extensions#pg_durable), not of point-in-time recovery, which uses physical backups and preserves workflow state.
:::
...
1. Go to the target project's [Backups](https://app.nhost.io/orgs/_/projects/_/backups) section and open the "Import Backup" tab.
