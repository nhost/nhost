...
Nhost preloads pg_durable and configures its background worker for your project database. The extension does not grant access to `PUBLIC`. To enable it and allow workflows submitted through Nhost GraphQL, create a migration that grants access to the `nhost_hasura` database role:

```sql
SET ROLE postgres;
CREATE EXTENSION pg_durable;
SELECT df.grant_usage('nhost_hasura');
RESET ROLE;
```
...
Nhost leaves `pg_durable.enable_superuser_instances` at its secure default of `off`. Workflows must therefore be submitted by a non-superuser role with `LOGIN`, because the background worker reconnects as the role captured by `df.start()`. Do not submit as `postgres` or `nhost_admin`; both are superusers and `df.start()` rejects them. The `nhost_hasura` role used by Nhost GraphQL is a non-superuser `LOGIN` role.

For a service that connects directly to PostgreSQL, use a dedicated, least-privileged login role in the installation migration. For example:
...
Nhost builds pg_durable with the `http-allow-all` feature, enabling unrestricted HTTP destinations. `df.http()` and `df.http_multipart()` can contact any HTTP or HTTPS destination reachable from PostgreSQL. However, build-time HTTP support does not grant SQL access automatically. Fresh pg_durable installations revoke execution of both HTTP functions from `PUBLIC`, and the general grant above does not include them. A database administrator must explicitly opt a role into HTTP egress:
...
Equivalent direct `GRANT EXECUTE` statements can grant either HTTP function more narrowly. Do not grant HTTP execution to roles that do not require outbound network access.
