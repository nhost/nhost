...
head:
  - tag: title
    content: "Firing Webhooks on Database Changes with Event Triggers | Nhost Docs"
sidebar:
...
| **Configure Transformation**   | Reshape the request with [Kriti templates](/reference/kriti) (see [Transformations](/products/events/transformations)) |
...
:::note
The webhook URL uses `{{ENV_VAR}}` environment-variable interpolation. This is different from the [Kriti templates](/reference/kriti) used in transformations, which evaluate expressions over bound values like `$body`.
:::
