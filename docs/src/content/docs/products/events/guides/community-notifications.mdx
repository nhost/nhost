...
description: Notify community members when a community's description is updated using an event trigger
keywords: ["event trigger", "community", "notifications", "webhook handler", "example"]
head:
  - tag: title
    content: "Notify Members on Updates with an Event Trigger | Nhost Docs"
sidebar:
  label: Community Notifications (Event Trigger)
---

An event trigger fires when a community's description is updated. The handler notifies all members except the editor. You can see this in action in the [react-demo](https://github.com/nhost/nhost/tree/main/examples/demos/react-demo) example, see [`Communities.tsx`](https://github.com/nhost/nhost/blob/main/examples/demos/react-demo/src/pages/Communities.tsx).
...
export default async (req: Request, res: Response) => {
  ...
  // session_variables contains the session variables of the user who made the change
...
- [Full source code](https://github.com/nhost/nhost/blob/main/examples/demos/backend/functions/events/community-updated.ts) on GitHub
