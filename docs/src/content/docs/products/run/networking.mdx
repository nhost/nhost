---
title: Networking
description: Connect Nhost Run services to Postgres, GraphQL, Auth, and Storage over a shared network, and expose HTTP and GRPC ports.
keywords: ["networking", "service ports", "internal networking", "GRPC", "HTTP endpoints", "service connectivity"]
head:
  - tag: title
    content: "Connect Run Services & Expose Ports | Nhost Docs"
---
...
- auth: base URL is `http://hasura-auth-service:4000`
- storage: base URL is `http://hasura-storage-service:5000`

When `experimental.nhost` enables engine mode, auth, storage, and the
Constellation GraphQL service share the engine container and port. Use these
engine-mode base URLs instead of the standalone service names:

| Service | Engine-mode base URL |
|---------|----------------------|
| auth | `http://engine:8080/auth/v1` |
| storage | `http://engine:8080/storage/v1` |
| graphql | `http://engine:8080/graphql/v1` |

The standalone `hasura-auth-service`, `hasura-storage-service`, and
`constellation-service` names do not resolve to the engine container.
...
:::note
Currently, only services of type `http` and `grpc` can be exposed to the internet.
:::
