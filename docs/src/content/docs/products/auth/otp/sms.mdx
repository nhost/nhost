...
When SMS OTP is enabled, signed-in users with a verified phone number can also use it to [elevate their session](/products/auth/elevated-permissions) before performing sensitive actions:
...
Both endpoints require [elevated permissions](/products/auth/elevated-permissions) when `auth.elevatedPrivileges.mode` is `required`, or when it is `recommended` and the user has at least one [elevation method](/products/auth/elevated-permissions#elevation-methods) available. With the default `disabled` mode no elevated claim is needed.
