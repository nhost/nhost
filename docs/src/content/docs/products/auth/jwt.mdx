...
description: Configure how Nhost Auth signs JWT access tokens using symmetric keys, asymmetric keys, or an external signing service.
keywords: ["JWT", "JSON Web Token", "access token", "refresh token", "claims", "token expiration", "bearer token"]
head:
  - tag: title
    content: "Configuring Access Tokens and Signing Keys | Nhost Docs"
---
...
### Claims Mapping and Namespace

When verifying tokens — especially those issued by an external provider — you can remap their claims onto the session variables your permissions expect, and control where those claims are read from. The `claims_namespace` defaults to `https://hasura.io/jwt/claims` (a [retained identifier](/products/graphql/configuration#naming-conventions) kept for compatibility) and can be overridden with `claims_namespace` or `claims_namespace_path`. Use `claims_map` to project individual claims from a literal value or a JSON path within the token.
...
```toml
[[hasura.jwtSecrets]]
jwk_url = "https://my-auth-server.com/.well-known/jwks.json"
...
The complete `#JWTSecret` schema, with every field and default, is documented in the [configuration reference](/reference/configuration#jwtsecret).
