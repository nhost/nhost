...
- **Two grant types** — [Authorization Code](/products/auth/oauth2-provider/authorization-flow) (with PKCE for public clients) and [Device Authorization](/products/auth/oauth2-provider/device-flow) (for CLIs, IoT, and input-constrained devices)
...
| [RFC 8628](https://datatracker.ietf.org/doc/html/rfc8628) | Device Authorization Grant — For CLIs, IoT devices, and input-constrained devices |
...
For the device flow, set the verification page URL with the `AUTH_OAUTH2_PROVIDER_DEVICE_VERIFY_URL` environment variable (defaults to `{clientUrl}/oauth2/device`). See [Device Flow](/products/auth/oauth2-provider/device-flow) for details.
</TabItem>
</Tabs>
...
<CardGroup cols={2}>
  ...
  <Card title="Device Flow" icon="terminal" href="/products/auth/oauth2-provider/device-flow">
    Authenticate CLIs, IoT devices, and input-constrained devices
  </Card>
