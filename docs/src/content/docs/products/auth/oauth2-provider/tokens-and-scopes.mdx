...
head:
  - tag: title
    content: "OAuth2 Token Formats, Scopes, and Lifecycle | Nhost Docs"
sidebar:
...
### The `graphql:role:xxxx` Scope

The `graphql:role:xxxx` scope works like the `graphql` scope but **narrows the claims** to specific roles. This is useful when a third-party integration should only see a subset of the user's roles.

- The plain `graphql` scope acts as a superset — if a client is allowed `graphql`, any `graphql:role:xxxx` scope is implicitly permitted. The reverse is not true: having `graphql:role:xxxx` does not grant `graphql`. To restrict a client to specific roles, omit `graphql` and list individual `graphql:role:xxxx` scopes.
- Multiple `graphql:role:xxxx` scopes can be requested in a single authorization. The first becomes `x-hasura-default-role` and all of them become `x-hasura-allowed-roles`.
- The `graphql` and `graphql:role:xxxx` scopes are **mutually exclusive** in a single request. A client may have both in its allowed scopes, but a request cannot use both at once.
- At token time, every requested role must exist in the user's allowed roles. If the user lacks a requested role, the token exchange fails with `access_denied`.
- On refresh, the original `graphql:role:xxxx` scopes are preserved and roles are re-validated against the user's current roles. If the user has lost a requested role since the original authorization, the refresh fails with `access_denied`.

Example: a client with `scopes: ["openid", "graphql:role:viewer"]` requesting `scope=openid graphql:role:viewer` produces:
...
```json
{
  ...
  "iat": 1234567890,
  "exp": 1234568790,
  "scope": "openid graphql:role:viewer",
  "https://hasura.io/jwt/claims": {
    "x-hasura-user-id": "f5765cb0-1c45-4b6e-8a30-0b2abc1a2f3d",
...
With multiple roles (`scope=openid graphql:role:viewer graphql:role:editor`):
...
```json
{
  "https://hasura.io/jwt/claims": {
    "x-hasura-user-id": "f5765cb0-1c45-4b6e-8a30-0b2abc1a2f3d",
    "x-hasura-default-role": "viewer",
    "x-hasura-allowed-roles": ["viewer", "editor"]
  }
}
```
...
When the `graphql` or `graphql:role:xxxx` scope is present, the UserInfo response also includes the GraphQL claims namespace with roles and any custom claims. With `graphql:role:xxxx`, the roles are narrowed the same way as in the access token. Unlike the access token (where session-variable values are string-encoded), the UserInfo endpoint returns proper JSON types — arrays as JSON arrays, booleans as booleans, etc. This makes it suitable for third-party OIDC consumers (e.g. Grafana) that read user attributes from the UserInfo endpoint.
