...
Confidential clients support both the [Authorization Code flow](/products/auth/oauth2-provider/authorization-flow) and the [Device Authorization flow](/products/auth/oauth2-provider/device-flow). The client authenticates with its secret in both cases.
