...
head:
  - tag: title
    content: "OAuth2 for Server-Side Apps with Client Secrets | Nhost Docs"
sidebar:
...
:::note
Some apps make the token and userinfo requests server-to-server. If the app runs in a container alongside Nhost, use the mode-appropriate internal auth base URL (`http://hasura-auth-service:4000` with standalone services or `http://engine:8080/auth/v1` in engine mode; see [Run networking](/products/run/networking)) for these endpoints, and the public URL for the authorization endpoint (which the browser accesses).
:::
