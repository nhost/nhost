...
head:
  - tag: title
    content: "Create and Configure OAuth2 Clients | Nhost Docs"
sidebar:
...
| `graphql` | Adds GraphQL-compatible claims (`https://hasura.io/jwt/claims`) to access tokens with all user roles |
| `graphql:role:xxxx` | Like `graphql`, but narrows claims to only the specified role(s). See [Tokens and Scopes](/products/auth/oauth2-provider/tokens-and-scopes/#the-graphqlrolexxxx-scope) for details. |

When creating a client, the `scopes` array defines which scopes the client is **allowed to request**. If no scopes are specified, the default is the standard scopes (`openid`, `profile`, `email`, `phone`, `offline_access`, `graphql`). To allow `graphql:role:xxxx` scopes, add them explicitly — for example:

```json
{
  "scopes": ["openid", "profile", "graphql:role:viewer", "graphql:role:editor"]
}
```

The plain `graphql` scope acts as a superset — if a client is allowed `graphql`, any `graphql:role:xxxx` scope is implicitly permitted. The reverse is not true: having `graphql:role:xxxx` does not implicitly grant `graphql`. To restrict a client to specific roles only, omit `graphql` and list the individual `graphql:role:xxxx` scopes instead.
