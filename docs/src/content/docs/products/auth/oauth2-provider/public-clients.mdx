...
Public clients support two grant types:

- **[Authorization Code with PKCE](/products/auth/oauth2-provider/authorization-flow)** — for apps that can handle browser redirects (SPAs, mobile apps with deep links)
- **[Device Authorization](/products/auth/oauth2-provider/device-flow)** — for input-constrained devices and CLI tools that cannot easily handle redirects
