...
sequenceDiagram
	...
	U->>C: Enter email
	C->>+A: POST /signin/passwordless/email-otp
	Note right of C: email
	opt No user found
		A->>A: Create user
	end
	A->>A: Generate OTP (expires in 5 min)
	A-)E: Send OTP email
	A->>-C: OK
	E-)U: Receive OTP via email
	U->>C: Enter OTP code
	C->>+A: POST /signin/passwordless/email-otp/verify
	Note right of C: email + OTP code
	A->>A: Verify OTP
