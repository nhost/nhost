...
description: Customize transactional auth emails like verification, password reset, and OTP using deployable HTML templates.
keywords: ["email templates", "transactional emails", "email customization", "SMTP", "email design", "verification email", "password reset email"]
head:
  - tag: title
    content: "Customizing Transactional Auth Emails | Nhost Docs"
---
...
Email Templates are deployed as part of a [Deployment](/platform/cloud/deployments) so your project must be connected to a Git repository.
...
Additionally, if you want to use passwordless sign-in or email OTP for sign-in and session elevation, you need to create the following templates as well:
