...
keywords: ["bot protection", "CAPTCHA", "Turnstile", "Cloudflare", "spam prevention", "security", "automated attacks"]
---
...
1. **Selective Protection**: Auth integrates Turnstile specifically for:
  a. Signup requests via email/password, passwordless email, email OTP, passwordless SMS, ID token, and WebAuthn
  b. Password reset requests
  c. Signin requests that trigger an email or SMS: magic-link email, email OTP, and SMS OTP
2. **API Accessibility**: Other API endpoints remain accessible for legitimate programmatic use. The following are explicitly excluded, so clients don't need to deliver a Turnstile token on them:
  a. Verification endpoints (for example `/signin/otp/email/verify`, `/signup/webauthn/verify`, `/signin/passwordless/sms/otp`)
  b. OAuth2 callback endpoints (`/signin/provider/{provider}/callback`)
  c. OAuth2 redirect initiators (`/signin/provider/{provider}` and `/signup/provider/{provider}`) — these are GET redirects that the browser navigates to via `window.location`, which cannot carry custom headers
