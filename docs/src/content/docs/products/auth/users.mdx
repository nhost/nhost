---
title: Users
description: Manage user accounts in Nhost Auth, including default and allowed roles, metadata, and GraphQL permissions on the auth schema.
keywords: ["users", "user management", "roles", "metadata", "profile", "account", "user data"]
head:
  - tag: title
    content: "Managing User Accounts, Roles, and Metadata | Nhost Docs"
---
...
await nhost.auth.signUp({
  ...
  options: {
    metadata: {
      birthYear: 1989,
      town: 'Stockholm',
      likes: ['Postgres', 'GraphQL', 'Authentication', 'Storage', 'Serverless Functions']
    }
  }
})
```
