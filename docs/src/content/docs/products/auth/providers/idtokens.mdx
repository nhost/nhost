...
description: Sign in users with OIDC ID tokens in Nhost Auth — verify Apple and Google identity tokens for native mobile authentication.
keywords: ["ID token", "OpenID Connect", "OIDC", "native authentication", "Apple Sign-In", "Google Sign-In", "identity provider"]
head:
  - tag: title
    content: "Native Authentication with OIDC ID Tokens | Nhost Docs"
sidebar:
  label: IDTokens
---
...
:::note
To use ID tokens, you need to:
1. Configure supported identity providers (currently [apple](/products/auth/providers/sign-in-apple) and [google](/products/auth/providers/sign-in-google))
2. Set the `audience` field in the provider configuration to match the audience claim in the ID token

Multiple audiences are supported—simply enter them separated by commas.
:::
