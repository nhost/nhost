---
title: Connect a Provider to an Existing User
description: Link OAuth providers to existing Nhost Auth accounts — let users connect multiple social logins to one user, even with different emails.
keywords: ["provider connect", "link accounts", "multiple providers", "OAuth", "user authentication", "account linking"]
head:
  - tag: title
    content: "Link OAuth Providers to Existing Accounts | Nhost Docs"
sidebar:
  label: Connecting existing users
---
