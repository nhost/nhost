---
title: Sign In with Google
description: Add Google OAuth social login to your app with Nhost Auth — create credentials, configure the provider, and sign in users.
keywords: ["Google", "Google login", "Google OAuth", "social login", "Google sign in", "Google authentication"]
head:
  - tag: title
    content: "Google OAuth Authentication Setup | Nhost Docs"
---

<p align="center">
  <img
    alt="Google Sign In Preview"
    src="/images/auth/google-signin.svg"
    width={480}
    height={267}
  />
</p>

## Sign Up for Google

- Sign up for [Google Cloud](https://cloud.google.com/free) if you don't have one already.

## Create a Google Cloud Project

:::tip
You can skip this step if you already have a Google Cloud project you want to use.
:::

- Create a new Google Cloud project if you don't already have a project you want to use.

## Configure OAuth Consent Screen

- Search for **OAuth consent screen** in the top search bar in the Google Cloud Console.
- Click on **OAuth consent screen** in the search results.
- Select User Type **External** and click **CREATE**.

## **Edit app registration**

### OAuth Consent Screen

- Fill in your App information.
- Click **SAVE AND CONTINUE**.

### Scopes

- Click **SAVE AND CONTINUE**.

### Test User

- Click **SAVE AND CONTINUE**.

### Summary

- Click **BACK TO DASHBOARD**.

## Create Credentials

- Click on **Credentials** under **APIs & Services** in the left menu.
- Click **+ CREATE CREDENTIALS** and then **OAuth client ID** in the top menu.
- On the **Create OAuth client ID** page for **Application Type** select **Web application**.
- Under **Authorized JavaScript origins**, add your project's redirect URL for the Google sign-in method in the following format: `https://<subdomain>.auth.<region>.nhost.run`
- Under **Authorized redirect URIs** add your **OAuth Callback URL** from Nhost.
- Click **CREATE**.

## Configure Nhost

- A modal appears with your Google Client ID and Client Secret.
- Copy and paste the **Client ID** and **Client Secret** from Google to your Nhost OAuth settings for Google. Make sure the OAuth provider is enabled in Nhost.
- Click the checkbox “**I have pasted the redirect URI into Google”**.
- Click **Confirm settings**.

## Configure Audience (Optional)

To enable sign-in with [ID tokens](/products/auth/providers/idtokens) (for native authentication on mobile and supported devices), set the `audience` field in your Google provider configuration to match the audience claim in the ID token. Multiple audiences can be specified by separating them with commas.

## Sign In Users

Once you've configured Google as an OAuth provider in Nhost, you can sign in users using the Google provider. See the [OAuth Provider Sign-In Guide](/products/auth/providers/sign-in-provider) for detailed implementation instructions including the complete OAuth flow, error handling, and session management.
