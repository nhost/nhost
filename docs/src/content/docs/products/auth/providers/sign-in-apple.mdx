...
description: Set up Sign in with Apple in Nhost Auth — configure your Apple ID OAuth credentials and authenticate iOS and web users.
keywords: ["Apple", "Apple login", "Apple OAuth", "Sign in with Apple", "iOS login", "Apple ID"]
head:
  - tag: title
    content: "Apple ID OAuth Authentication Setup | Nhost Docs"
---

<p align="center">
  <img
    alt="Apple Sign In Preview"
    src="/images/auth/apple-signin.svg"
    width={480}
    height={267}
  />
</p>
...
To enable sign-in with [ID tokens](/products/auth/providers/idtokens) (for native authentication on mobile and supported devices), set the `audience` field in your Apple provider configuration to match the audience claim in the ID token. Multiple audiences can be specified by separating them with commas.
