...
description: Configure the client URL and allowed redirect URLs that Nhost Auth uses to send users back to your app after auth flows.
keywords: ["client URL", "redirect URLs", "authentication flow", "frontend integration", "callback URLs", "auth redirect"]
head:
  - tag: title
    content: "Configuring Auth Callback and Redirect URLs | Nhost Docs"
---
