...
description: Sign users in with WebAuthn passkeys and security keys like Touch ID, Face ID, Windows Hello, and YubiKey.
keywords: ["WebAuthn", "security key", "passkey", "FIDO2", "biometric", "fingerprint", "face ID", "YubiKey", "passwordless", "PKCE"]
head:
  - tag: title
    content: "Passkey and Security Key Login with WebAuthn | Nhost Docs"
sidebar:
...
await nhost.auth.verifySignInWebauthn({ credential });
```
...
const response = await nhost.auth.addSecurityKey();
