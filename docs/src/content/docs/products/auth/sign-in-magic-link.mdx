...
To sign in users with a Magic Link, use `signInPasswordlessEmail` with an email and a `codeChallenge` for [PKCE](/products/auth/sign-in-email-password#pkce):
...
After the user clicks the magic link in their email, they'll be redirected to your app with an authorization code. See [Handling the Verification Redirect](/products/auth/sign-in-email-password#handling-the-verification-redirect) to exchange the code for a session.
