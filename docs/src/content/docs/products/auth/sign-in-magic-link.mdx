...
description: Sign users in with passwordless magic links emailed to them, secured with PKCE and no password required.
keywords: ["magic link", "passwordless", "email login", "link authentication", "no password", "email sign in", "PKCE"]
head:
  - tag: title
    content: "Passwordless Email Login with Magic Links | Nhost Docs"
sidebar:
  label: Magic Links
---
