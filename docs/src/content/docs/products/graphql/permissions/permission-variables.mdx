...
head:
  - tag: title
    content: "Session Variables & Roles in Permission Rules | Nhost Docs"
sidebar:
...
:::note[Why the `x-hasura-` prefix?]
Session variables use the `x-hasura-` prefix (for example `x-hasura-user-id` and `x-hasura-role`), a [retained naming convention](/products/graphql/configuration#naming-conventions) kept for compatibility. Keep it as-is.
:::
