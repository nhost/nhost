...
Session variables use the `x-hasura-` prefix (for example `x-hasura-user-id` and `x-hasura-role`). This is a naming convention kept for compatibility with existing tooling and metadata — keep it as-is.
:::
