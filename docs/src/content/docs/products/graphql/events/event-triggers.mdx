...
The webhook target can be any HTTP endpoint that returns a 2xx status code. By default, events are delivered as POST requests, but you can change the HTTP method using [request transformations](#request-and-payload-transformations). A common pattern is to use an Nhost Function for async business workflows.
...
Override the default POST method (GET, PUT, PATCH, DELETE), customize the URL path with templates (`{{$base_url}}/custom-path`), and add query parameters as key-value pairs or a URL string template.
...
![Additional Headers for Event triggers](/images/graphql/events/additional-headers.png)
