...
![Create Cron Trigger](/images/graphql/events/cron-trigger-create.png)
...
Cron triggers deliver an HTTP request to any endpoint. The default method is POST, but you can change it using [request transformations](/products/graphql/events/event-triggers/#request-and-payload-transformations). The request body contains a `payload` field with the JSON you configured, not database row data.
...
For webhook security details, see [Event Triggers — Webhook Security](/products/graphql/events/event-triggers#webhook-security). For using the Nhost SDK inside handlers, see the [SDK guide](/products/functions/guides/nhost-sdk).
...
For full details on available transform types and template variables, see [Event Triggers — Request and Payload Transformations](/products/graphql/events/event-triggers#request-and-payload-transformations).
...
![Cron Trigger Overview](/images/graphql/events/cron-trigger-overview.png)
...
![Cron Trigger Events](/images/graphql/events/cron-trigger-events.png)
