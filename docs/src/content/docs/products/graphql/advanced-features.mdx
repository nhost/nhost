...
description: Advanced GraphQL API controls — query-level observability, depth limits, rate limiting, and security against DoS attacks.
keywords: ["GraphQL", "security", "observability", "query depth", "admin secret", "rate limiting", "DoS protection"]
head:
  - tag: title
    content: "GraphQL API Security & Observability Controls | Nhost Docs"
---
