...
Add extra headers as static values or from environment variables. Use environment variables for sensitive values like API keys so you can rotate them without editing the Action.
...
Open the `⋯` menu for an Action and select **Edit Relationships**. Click **Relationship +** to add a new relationship, or the edit icon <SquarePen size={16} style="display: inline; vertical-align: middle; margin: 0;" /> on an existing one, to open the dialog.

![Relationship dialog with the reference table and field mapping](/images/products/graphql/actions/edit-relationship-modal.png)

<Steps>

1. **Choose the relationship type**

   Pick an **Object Relationship** (a single related row) or an **Array Relationship** (a list of related rows).

2. **Select the reference table**

   Choose the database table to relate to.

3. **Map the fields**

   Map each field of the Action's output type to a column on the reference table.

4. **Save**

   The relationship becomes available on the Action's response type in your GraphQL schema.
