...
keywords: ["Remote Schemas", "GraphQL", "schema stitching", "API integration", "microservices", "third-party APIs", "Hasura remote schemas"]
head:
  - tag: title
    content: "Merge External GraphQL Services into Your API | Nhost Docs"
sidebar:
...
- **Third-party integrations**: Connect to external GraphQL APIs like payment processors, CMS platforms, or analytics services. For example, use [`@nhost/stripe-graphql-js`](/products/graphql/guides/stripe) to query Stripe data directly from your GraphQL API
- **Microservices architecture**: Unify multiple internal GraphQL services under one endpoint
- **Custom GraphQL services on Nhost Run**: Build your own GraphQL servers and deploy them on [Nhost Run](/products/run), then connect them via Remote Schemas. For example, you could create a dedicated billing service that handles subscription management and expose it through your main GraphQL API
- **Legacy system integration**: Expose existing services through your GraphQL API

## Adding a Remote Schema

![Add Remote Schema](/images/graphql/remote-schemas-create.png)
...
:::note
You can use environment variables in the URL using the `{{VARIABLE_NAME}}` syntax. For example: `https://api.example.com/graphql?key={{API_KEY}}`. See [Environment Variables](/platform/cloud/environment-variables) for more details.
:::
...
![GraphQL Customizations](/images/graphql/remote-schemas-customizations.png)
...
![permission](/images/graphql/remote-schemas-settings-permissions.png)
...
2. **Go to Permissions**
   ...
   ![Edit Permissions menu](/images/graphql/remote-schemas-permissions-edit.png)

3. **Edit permissions for a role**

   Click on the **Permission** cell for the role you want to configure.

   ![Permissions matrix](/images/graphql/remote-schemas-permissions-cell.png)
   ...
   ![Permission roles configuration](/images/graphql/remote-schemas-permissions-roles.png)
...
You can preset an argument's value per role with the `@preset` directive. The value is auto-injected at request time and the argument is hidden from that role's schema.

Expand a field's row to reveal its arguments; each one has a preset input with an Insert-literal dropdown (the `{}` icon). The dropdown is gated by the argument's type and offers `null`, `""`, `true` / `false`, enum values, and your project's [permission variables](/products/graphql/permissions/permission-variables) (`X-Hasura-*`). You can also type the literal directly.

![Argument preset dropdown](/images/graphql/remote-schemas-permissions-preset-dropdown.png)
