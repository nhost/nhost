...
description: "Build a Next.js app with Nhost in minutes: connect the SDK, run GraphQL queries, and add authentication with server-side rendering support."
keywords: ["Next.js", "quickstart", "React", "Nhost SDK", "GraphQL", "server-side rendering"]
head:
  - tag: title
    content: "Next.js Backend Quickstart Guide | Nhost Docs"
sidebar:
  label: Next.js
---
...
Create the application using standard tooling:
    ...
    ```bash Terminal
    npx create-next-app@latest my-app --yes --js --src-dir
    cd my-app
    ```
...
- [Full source code](https://github.com/nhost/nhost/tree/main/examples/quickstarts/nextjs) on GitHub — a complete example configured for a local Nhost project
