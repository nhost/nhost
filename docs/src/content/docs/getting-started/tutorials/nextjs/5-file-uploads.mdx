...
description: "Add file uploads to your Next.js app with Nhost Storage: configure buckets and permissions and build a full file management system."
keywords: ["Next.js", "file uploads", "storage buckets", "file management", "server components", "Nhost storage"]
head:
  - tag: title
    content: "Manage File Storage and Uploads in Next.js | Nhost Docs"
sidebar:
  label: File Uploads
---
...
2. Click **New Bucket** in the sidebar
3. Set the name to `personal` and click **Save**
...
2. Enable the **Uploader identity** toggle — this automatically sets `uploaded_by_user_id` to the current user's ID on every upload, so you can reference it in Download and Delete rules.
...
1. Select **With custom check** and add the same two rules as Download:
   - `bucket_id` equals `personal`
   - `uploaded_by_user_id` equals `X-Hasura-User-Id`
...
```tsx title="src/app/files/actions.ts" showLineNumbers
"use server";
...
```tsx title="src/app/files/download/[fileId]/route.ts" showLineNumbers
...
```tsx title="src/app/files/page.tsx" showLineNumbers
import type { FileMetadata } from "@nhost/nhost-js/storage";
import { createNhostClient } from "../../lib/nhost/server";
...
```tsx title="src/app/files/FilesClient.tsx" showLineNumbers
"use client";
...
### Increase the Next.js Server Action Upload Limit

:::note
By default, Next.js Server Actions only accept request bodies up to 1 MB. 
In Next.js 15 and 16, you can increase this limit by configuring <a href='https://nextjs.org/docs/app/api-reference/config/next-config-js/serverActions#bodysizelimit' target='_blank' rel="noopener noreferrer">experimental.serverActions.bodySizeLimit</a> in `next.config.ts`.
:::

To increase the upload limit, we want to update our Next.js config file.


```tsx title="next.config.ts" showLineNumbers {4-8}
import type { NextConfig } from "next";

const nextConfig: NextConfig = {
  experimental: {
    serverActions: {
      bodySizeLimit: "10mb", // increases limit to 10MB
    },
  },
};
...
Adjust the limit as needed (2mb, 5mb, 25mb, etc).

### Update Navigation Component

Add a link to the files page in the server-side navigation component.

```tsx title="src/components/Navigation.tsx" showLineNumbers {26-28}
...
<details>
<summary>Server/Client Architecture</summary>
...
<details>
<summary>Storage Bucket & Permissions</summary>
...
<details>
<summary>Server-Side Data Fetching</summary>
