...
description: >-
  Learn how to protect routes and content in a Next.js application using Nhost
  authentication.
keywords: ["Next.js", "protected routes", "authentication", "App Router", "server components", "session management"]
head:
  - tag: title
    content: "Secure Next.js App Router Pages with Auth | Nhost Docs"
sidebar:
...
```bash
npx create-next-app@16 nhost-nextjs-tutorial --typescript --eslint --app --no-tailwind --src-dir --yes
...
Create server-side utilities for handling authentication in Next.js server components and proxy. This enables server-side session access and token refresh functionality.

```tsx title="src/lib/nhost/server.tsx" showLineNumbers
...
### Create Proxy for Route Protection

Create a Next.js proxy to handle route protection at the server level. This proxy runs before any page renders and automatically redirects unauthenticated users from protected routes.

```tsx title="src/proxy.ts" showLineNumbers
...
Create a page that displays user information. Note that the page itself doesn't need any special authentication logic - the route protection is handled entirely by the proxy from the previous step.

```tsx title="src/app/profile/page.tsx" showLineNumbers
...
```tsx title="src/components/Navigation.tsx" showLineNumbers
...
```css title="src/app/globals.css" showLineNumbers
:root {
...
1. **Server-Side Helpers**: Utilities for handling authentication in Next.js server components and proxy
2. **Proxy Route Protection**: Next.js proxy runs before any page renders, automatically redirecting unauthenticated users from protected routes and refreshing tokens
3. **Protected Pages**: Server components can assume authentication since the proxy handles protection, focusing purely on rendering authenticated content
4. **Navigation**: Server-side navigation component that adapts its links based on authentication status
5. **Automatic Redirects**: All route protection and redirects are handled at the proxy level for optimal performance and security

## Key Features Demonstrated


<details>
<summary>Proxy Route Protection</summary>

Routes are protected at the proxy level, running before any page renders to provide the highest level of security and performance by preventing unauthorized access before components load.

</details>

<details>
<summary>No Loading States</summary>
...
<details>
<summary>Automatic Server Redirects</summary>

Users are automatically redirected based on their authentication status using Next.js proxy redirects, ensuring proper navigation flow before any client-side rendering.
...
<details>
<summary>Cookie-Based Session Management</summary>
...
<details>
<summary>Automatic Token Refresh</summary>

The proxy automatically refreshes authentication tokens before they expire, ensuring seamless user experience without manual token management.
