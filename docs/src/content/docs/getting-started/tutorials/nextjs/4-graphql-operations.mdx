...
description: >-
  ...
  building a complete todos application with Nhost and Next.js
keywords: ["Next.js", "GraphQL", "CRUD operations", "server actions", "todos app", "App Router"]
head:
  - tag: title
    content: "Query and Mutate Data with GraphQL in Next.js | Nhost Docs"
sidebar:
...
```tsx title="src/app/todos/page.tsx" showLineNumbers
...
export default async function TodosPage() {
  ...
  if (session) {
    try {
      ...
      // The query automatically filters by user_id due to permissions
...
```tsx title="src/app/todos/TodosClient.tsx" showLineNumbers
"use client";
...
```tsx title="src/app/todos/actions.ts" showLineNumbers
"use server";
...
export async function addTodo(data: {
  ...
  try {
    ...
    // user_id is automatically set based on the JWT token
...
export async function updateTodo(
  ...
  try {
    ...
    // Permissions ensure users can only update their own todos
...
export async function deleteTodo(id: string): Promise<ActionResult> {
  ...
  try {
    ...
    // Permissions ensure users can only delete their own todos
...
```tsx title="src/components/Navigation.tsx" showLineNumbers {23-25}
...
<details>
<summary>Type-Safe Server Actions</summary>
