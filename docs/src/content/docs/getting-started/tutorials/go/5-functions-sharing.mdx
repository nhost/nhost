...
:::note
Unlike the previous parts, `export` needs the function actually running.
Functions are served by your **local dev environment** — start it with `nhost up`
and it serves your `functions/` directory alongside the same auth, database, and
storage the CLI already uses. To run against a **cloud** project instead, connect
a GitHub repo and push to deploy (see the
