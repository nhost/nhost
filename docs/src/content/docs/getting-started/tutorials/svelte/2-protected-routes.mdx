...
head:
  - tag: title
    content: "Secure SvelteKit Routes with Auth | Nhost Docs"
sidebar:
...
```ts title="src/lib/nhost/auth.ts" showLineNumbers
...
```svelte title="src/routes/profile/+page.svelte" showLineNumbers
<script lang="ts">
import { goto } from "$app/navigation";
