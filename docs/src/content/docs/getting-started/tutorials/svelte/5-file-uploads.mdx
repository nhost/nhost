...
description: "Add file uploads to your SvelteKit app with Nhost Storage: configure buckets and permissions and build a file management system."
keywords: ["SvelteKit", "file uploads", "storage buckets", "file management", "storage permissions", "Nhost storage"]
head:
  - tag: title
    content: "Manage File Storage and Uploads in SvelteKit | Nhost Docs"
sidebar:
  label: File Uploads
---
...
```svelte title="src/routes/files/+page.svelte" showLineNumbers
<script lang="ts">
...
```svelte title="src/routes/+layout.svelte" showLineNumbers {40}
<script lang="ts">
