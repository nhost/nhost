...
description: Run GraphQL queries and mutations and set database permissions while building a complete todos app with Nhost and SvelteKit.
keywords: ["SvelteKit", "GraphQL", "CRUD operations", "permissions", "todos app", "mutations", "queries"]
head:
  - tag: title
    content: "Query and Mutate Data with GraphQL in SvelteKit | Nhost Docs"
sidebar:
...
```svelte title="src/routes/todos/+page.svelte" showLineNumbers
<script lang="ts">
...
```svelte title="src/routes/+layout.svelte" showLineNumbers {39}
<script lang="ts">
