...
description: >-
  ...
  building a complete todos application with Nhost and Vue
keywords: ["Vue", "GraphQL", "CRUD operations", "permissions", "todos app", "mutations", "queries"]
head:
  - tag: title
    content: "Query and Mutate Data with GraphQL in Vue | Nhost Docs"
sidebar:
...
```vue title="src/views/Todos.vue" showLineNumbers
<template>
...
```ts title="src/router/index.ts" showLineNumbers {7,39-44}
...
```vue title="src/components/Navigation.vue" showLineNumbers {14-16}
<template>
