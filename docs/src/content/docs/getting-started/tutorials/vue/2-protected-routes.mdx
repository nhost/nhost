...
description: >-
  ...
  authentication.
keywords: ["Vue", "protected routes", "authentication", "Vue Router", "session management", "route guards"]
head:
  - tag: title
    content: "Secure Vue Router Pages with Auth | Nhost Docs"
sidebar:
...
```typescript title="src/lib/nhost/auth.ts" showLineNumbers
...
```vue title="src/views/ProfileView.vue" showLineNumbers
<template>
...
```vue title="src/views/HomeView.vue" showLineNumbers
<template>
  <div class="container">
    <header class="page-header">
...
```vue title="src/components/Navigation.vue" showLineNumbers
<template>
...
```typescript title="src/router/index.ts" showLineNumbers
