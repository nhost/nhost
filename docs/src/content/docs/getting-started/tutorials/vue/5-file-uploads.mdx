...
description: "Add file uploads to your Vue app with Nhost Storage: configure buckets and permissions and build a complete file management system."
keywords: ["Vue", "file uploads", "storage buckets", "file management", "storage permissions", "Nhost storage"]
head:
  - tag: title
    content: "Manage File Storage and Uploads in Vue | Nhost Docs"
sidebar:
  label: File Uploads
---
...
```vue title="src/views/Files.vue" showLineNumbers
<template>
  <div class="container">
...
```ts title="src/router/index.ts" showLineNumbers {3,46-51}
...
```vue title="src/components/Navigation.vue" showLineNumbers {17-19}
<template>
