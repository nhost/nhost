...
**--debug**: Enable verbose debug logging [$NHOST_DEBUG]
...
**--functions-version**="": Functions version to use (default: "2.1.0") [$NHOST_FUNCTIONS_VERSION]
...
Snapshot a bucket's files (create) or restore them (apply)

**--help, -h**: show help

#### create

Create a local seed by downloading all files in a bucket

**--admin-secret**="": Hasura admin secret [$NHOST_ADMIN_SECRET]

**--dir**="": Local files directory. Defaults to &lt;nhost-folder&gt;/files [$NHOST_STORAGE_DIR]

**--disable-tls**: Disable TLS when targeting the local development environment [$NHOST_DISABLE_TLS]

**--help, -h**: show help

**--http-port**="": HTTP(S) port for the local development environment (default: 443) [$NHOST_HTTP_PORT]

**--subdomain**="": Project subdomain. Use 'local' for the local development environment. Defaults to the linked project. [$NHOST_SUBDOMAIN]
...
Restore a local seed by uploading its files into a bucket
