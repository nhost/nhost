...
| `--auth-port=""` | If specified, expose auth on this port. Not recommended. Rejected in engine mode (experimental.nhost), where auth is mounted behind a path prefix; use the Traefik URLs instead. Default: `0`. |
...
| `--storage-port=""` | If specified, expose storage on this port. Not recommended. Rejected in engine mode (experimental.nhost), where storage is mounted behind a path prefix; use the Traefik URLs instead. Default: `0`. |
