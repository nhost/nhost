...
**--package-manager**="": Package manager for the frontend (pnpm, npm or bun) (default: "pnpm")

**--template, -t**="": Template to scaffold (default: "nextjs-shadcn")
...
Attach to a running development environment.
...
| `--yes, --no-interactive` | Skip confirmation. Env: `$NHOST_YES`, `$NO_INTERACTIVE`. |
...
| | |
| --- | --- |
| `--admin-secret=""` | Admin secret for local project (default: nhost-admin-secret). |
| `--cloud` | Enable Nhost Cloud access. |
| `--local` | Enable local development access. |
| `--project-admin-secret=""` | Cloud project admin secret. |
| `--project-pat=""` | Cloud project personal access token. |
| `--project-region=""` | Cloud project region. |
| `--project-subdomain=""` | Cloud project subdomain to configure. |
| `--yes, --confirm` | Skip confirmation prompt. Env: `$NHOST_YES`, `$CONFIRM`. |
...
| | |
| --- | --- |
| `--remote` | Initialize pulling configuration, migrations and metadata from the remote project. Env: `$NHOST_REMOTE`. |
...
| | |
| --- | --- |
| `--subdomain=""` | Subdomain of the project to link to. Env: `$NHOST_SUBDOMAIN`. |
