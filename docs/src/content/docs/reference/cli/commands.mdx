...
| | |
| --- | --- |
| `--branch=""` | Branch name used to namespace this project's docker volumes (detected from git by default). Default: `<current-git-branch>`. Env: `$BRANCH`. |
...
| `--project-name=""` | Project name. Default: `<nhost/project-name or directory name>`. Env: `$NHOST_PROJECT_NAME`. |
...
Create a new Nhost project from a template, in [directory] when one is given, otherwise in a directory named after the project.

| | |
| --- | --- |
| `--name=""` | Project name, and the directory it lands in (default: nhost-starter-app, or [directory]'s name). |
| `--no-install` | Skip installing frontend dependencies. |
| `--package-manager=""` | Package manager for the frontend (pnpm, npm, bun, yarn). Default: `"pnpm"`. |
| `--template, -t=""` | Template to scaffold (available: nextjs-shadcn). Default: `"nextjs-shadcn"`. |
| `--template-path=""` | Use a local template directory instead of downloading (`offline/dev`). |
...
| `--dashboard-version=""` | Dashboard version to use. Default: `"nhost/dashboard:3.5.3"`. Env: `$NHOST_DASHBOARD_VERSION`. |
...
| `--functions-version=""` | Functions version to use. Default: `"2.3.1"`. Env: `$NHOST_FUNCTIONS_VERSION`. |
...
| `--user=""` | User to run containers that write into bind mounts as. 'auto' maps them to your uid:gid on Linux with a local rootful docker daemon, 'none' keeps the images' default user, or pass an explicit uid:gid. Default: `"auto"`. Env: `$NHOST_DOCKER_USER`. |
