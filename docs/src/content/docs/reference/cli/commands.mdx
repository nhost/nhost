...
**--dashboard-version**="": Dashboard version to use (default: "nhost/dashboard:2.63.0") [$NHOST_DASHBOARD_VERSION]
...
**--admin-secret**="": Admin secret (defaults: local subdomain → nhost-admin-secret; cloud → fetched from Nhost API). NHOST_ADMIN_SECRET is consulted only in linked-project mode and never forwarded to --url.
...
**--role, -r**="": Role to generate the schema for (default: "user")

**--subdomain**="": Project subdomain (defaults to the linked project; use 'local' for the local dev stack) [$NHOST_SUBDOMAIN]
