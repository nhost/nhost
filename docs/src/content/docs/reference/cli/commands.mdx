...
| | |
| --- | --- |
| `--name=""` | Project name, and the directory it lands in (default: nhost-starter-app, or the target directory's name). |
...
| `--template, -t=""` | Template to scaffold (available: nextjs-shadcn, react-native, react, vue, svelte). Default: `"nextjs-shadcn"`. |
| `--template-path=""` | Use a local template directory instead of downloading (for offline or development use). |
