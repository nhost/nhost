...
Create a page that displays user information. Note that the page itself doesn't need any special authentication logic - the route protection is handled entirely by the middleware from earlier steps when we set up routing.
...
Replace the contents of the file `src/app/globals.css` with the following styles to provide a clean and modern look for the application:
