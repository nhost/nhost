...
This tutorial builds upon the previous GraphQL operations tutorial by demonstrating how to implement file upload functionality with proper storage permissions. You'll learn how to create storage buckets, configure upload permissions, and implement complete file management operations in a React application.

<Info>
This is **Tutorial 5** in the React Authentication Series. This tutorial focuses on file storage, upload operations, and permission-based file access control in a production application.
</Info>
...
**Current Tutorial** - File upload and management
</Card>
</CardGroup>

## Prerequisites

- Complete the [GraphQL Operations tutorial](/getting-started/tutorials/react/4-graphql-operations) first
...
4. As id set `public`, leave the rest of the fields blank and click on Insert at the bottom
...
Now we need to set up permissions for the storage bucket to ensure users can only upload, view, and delete their own files.
...
```tsx src/pages/Uploads.tsx lines
...
Add styles for the file upload interface to your existing CSS.
...
2. **File Upload**: Upload files of different types (images, documents, etc.)
3. **File Listing**: View your uploaded files in the table
4. **File Viewing**: Click the view button to open/download files
5. **File Deletion**: Delete files you no longer need
6. **Security**: Sign in with another account and verify you cannot see files from the first account
7. **Permissions**: Verify that only authenticated users can access the uploads page
</Step>
</Steps>
