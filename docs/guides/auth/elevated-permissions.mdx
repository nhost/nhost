...
However, we are requiring elevated permissions to `insert`, `upadte` and `delete` with the permissions `notes.user_id eq x-hasura-auth-elevated`. In our example we automatically initiate the elevation process if the claim isn't already present:
