...
```graphql
sendMessage {
   sesionID: String!  message: String!  prevMessageID: String! 
}: MessageResponse!
```
...
## Arguments
    ...
    <ParamField body="sesionID" type="String!" required>
...
query sendMessage (
  ...
  graphite {
    sendMessage (
      sesionID: $sesionIDmessage: $messageprevMessageID: $prevMessageID
    ) {
      sessionID
      messages {
        id
