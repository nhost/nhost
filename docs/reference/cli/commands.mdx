...
**--branch**="": Git branch name. If not set, it will be detected from the current git repository. This flag is used to dynamically create docker volumes for each branch. If you want to have a static volume name or if you are not using git, set this flag to a static value. (default: nogit)
...
**--dashboard-version**="": Dashboard version to use (default: nhost/dashboard:2.44.1)
...
**--docker-config**="": Path to docker config file (default: /homeless-shelter/.docker/config.json)
...
**--config-file**="": Configuration file path. Defaults to $NHOST_DOT_NHOST_FOLDER/nhost-mcp.toml
...
**--grouped**: Show pages organized by tab and section
