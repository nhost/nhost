---
title: 'Migration guide to our new JavaScript SDK' 
sidebarTitle: 'Migration Guide'
description: 'Learn how to migrate from the deprecated SDKs to the new unified JavaScript SDK'
---

This guide helps you migrate from our deprecated SDKs to the new unified JavaScript SDK (`@nhost/nhost-js` v4.x+), which is isomorphic and auto-generated.

In case you missed it, please read our [announcement blog post](https://nhost.io/blog/introducing-new-javascript-sdk) for more information.
...
## `@nhost/nhost-js` v3.x → v4.x

This section covers migrating from the old vanilla JavaScript SDK to the new one. If you're migrating from React/Next.js/Vue, skip to the section on [React](#%40nhost%2Freact-→-%40nhost%2Fnhost-js).
...
| `auth.resetPassword(email)` | `auth.sendPasswordResetEmail({ email })` | Options object |
| `auth.sendVerificationEmail(email)` | `auth.sendVerificationEmail({ email })` | Options object |
| `auth.getUser()` | `auth.getUser()` | Alternatively access via session |
| `auth.getSession()` | `getUserSession()` | Renamed method |
| `auth.getAccessToken()` | `getUserSession()?.accessToken` | Access via session |
| `auth.refreshSession()` | `refreshSession(marginSeconds)` | Added a margin parameter in seconds  |

### WebAuthn / Security Keys

| v3.x | v4.x | Notes |
|------|------|-------|
| `auth.addSecurityKey(nickname?)` | `auth.addSecurityKey()` + `auth.verifyAddSecurityKey()` | Two-step process, requires `@simplewebauthn/browser` |
| `auth.signInSecurityKey()` | `auth.signInWebauthn()` | Browser WebAuthn integration |
| `auth.signUp({ email, securityKey: true })` | `auth.signUpWebauthn()` + `auth.verifySignUpWebauthn()` | Two-step registration |
| `auth.elevateEmailSecurityKey(email)` | `auth.elevateWebauthn()` + `auth.verifyElevateWebauthn()` | For elevated permissions |
...
| v3.x | v4.x | Notes |
|------|------|-------|
| MFA machine with `generateQrCode()` | `auth.changeUserMfa()` | v3.x used state machines, v4.x uses direct methods |
| MFA machine with `activateMfa(code)` / `disableMfa(code)` | `auth.verifyChangeUserMfa({ activeMfaType: 'totp', code })` | Activate/disable MFA |
...
| `storage.download({ fileId })` | `storage.getFile(fileId)` | Renamed method, uses positional parameter |
| `storage.delete({ fileId })` | `storage.deleteFile(fileId)` | Renamed method, uses positional parameter |
| `storage.getPresignedUrl({ fileId })` | `storage.getFilePresignedURL(fileId)` | Renamed method, uses positional parameter |
| `storage.getPublicUrl({ fileId })` | `${nhost.storage.baseURL}/files/${fileId}` | Construct URL manually |

### GraphQL Methods

| v3.x | v4.x | Notes |
|------|------|-------|
| `graphql.request(query, variables)` | `graphql.request({ query, variables })` | Options object with `query` key |

### Functions Methods

| v3.x | v4.x | Notes |
|------|------|-------|
| `functions.call(path, body)` | `functions.post(path, body)` | Renamed method |

---

## `@nhost/react` → `@nhost/nhost-js`
...
<Note>
While this guide uses React, the same changes apply to other frameworks like Next.js and Vue.js. You'll need to adapt the provider patterns and state management to your framework (e.g., using Vue's `provide`/`inject` or composables instead of React Context and hooks).
</Note>
...
- Review the [Auth API Reference](/reference/overview)
...
3. **Monitor errors** - Watch for any runtime issues in production
