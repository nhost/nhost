...
This package creates a Stripe GraphQL API that can be used to interact with Stripe via GraphQL.

The package is also meant to be used alongside your existing GraphQL API to connect data from your database and Stripe.
...
```bash
pnpm install @nhost/stripe-graphql-js
```
...
Create a new [Serverless Function](/platform/serverless-functions): `functions/graphql/stripe.ts`:
...
Learn more about the [Nhost CLI](/platform/cli).
