...
You need a [Twillio account](https://www.twilio.com/try-twilio) to use this feature.

Activate SMS sign in in the Nhost dashboard under **Users** -> **Login settings** -> **Passwordless SMS**.
...
It's recommended to start all phone numbers with `+` (and not `00`) to follow the [E.164 formatting standard](https://en.wikipedia.org/wiki/E.164).
...
We only support Twillio for now. If you want another SMS provider, please create an issue on [Hasura Auth on GitHub](https://github.com/nhost/hasura-auth).