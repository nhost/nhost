...
Seed data is a way of automatically adding data to your database when you create a new environment. This is, for the moment, only applicable when you're using the [Nhost CLI](/platform/cli) to develop locally. When you're running `nhost up` for the first time, seed data is added.
...
```text
nhost/seeds/default/1-countries.sql
nhost/seeds/default/2-products.sql
```
