...
The two schemas `auth` and `storage` are reserved for [Nhost Auth](/platform/authentication) and [Nhost Storage](/platform/storage) to work. You're allowed to modify **permissions** and **add relationships**. However, never modify any tables or remove relationships that were added by Nhost inside the `auth` and `storage` schemas.
...
6. (Optional) Select an **identity** column (for integer columns only).
...
For projects older than September 2022, it's not possible to connect directly to the database via a connection string. We're working on a one-click fix this this and expect the fix to be available in October 2022.
...
Databases on the [Pro and Enterprise plans](https://nhost.io/pricing) are backed up automatically every day.
