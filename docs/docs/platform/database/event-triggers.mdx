...
Event Triggers enable you to invoke webhooks when a database event triggers. Event Triggers are typically used to do post-processing tasks, using custom backend code, after a database event has occurred.
...
Event Triggers are managed in the Hasura Console. Select **Events** in the main menu and press "Create".
...
Event Triggers and [Serverless Functions](/platform/serverless-functions) is a perfect combination to build powerful backend logic. Every Serverless Function is exposed as an HTTP endpoint that can be used as a webhook for your Event Triggers.
...
The `NHOST_BACKEND_URL` is a [system environment variable](/platform/environment-variables#system-environment-variables) and it's available in production and in development environments using the [CLI](/platform/cli).

### Security

To make sure incoming request to your webhook comes from Hasura, and not some malicious third party, you can use a shared webhook secret between Hasura and your webhook handler.

It's recommended to use the `NHOST_WEBHOOK_SECRET`, which is a [system environment variable](/platform/environment-variables#system-environment-variables) and it's available in production and in development environments using the [CLI](/platform/cli).
...
- Step 2: Check the header `nhost-webhook-secret` for the incoming request and make sure it is the same as the environment variable `NHOST_WEBHOOK_SECRET`.

Here's an example of how to check the header in your Serverless Function:
...
The `NHOST_WEBHOOK_SECRET` is a [system environment variable](/platform/environment-variables#system-environment-variables) and it's available in production and in development environments using the [CLI](/platform/cli).
