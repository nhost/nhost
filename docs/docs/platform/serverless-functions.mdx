...
You can prepend files and folders with `_` to prevent them from being treated as serverless functions. This is useful if you have a utils file (`functions/_utils.js`) or a utils folder (`functions/_utils/[multiple-files].js`).
