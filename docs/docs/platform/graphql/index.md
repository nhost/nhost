...
Every Nhost app has it's own autogenerated GraphQL API. The GraphQL API is based on the tables and columns in the [Postgres database](/platform/database) and is instanlty available. It's [Hasura GraphQL engine](https://github.com/hasura/graphql-engine) that powers the GraphQL API.

The GraphQL API is available at: `https://[subdomain].nhost.run/v1/graphql`.

## What is GraphQL

GraphQL is a query language for APIs that prioritize developer experience. The GraphQL API can be used to both fetch (query) and modify (mutation) data. GraphQL is particular powerful for frontend developers who wants to build products fast.
