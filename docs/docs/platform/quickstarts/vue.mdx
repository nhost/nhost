...
Le'ts also use the `useAuthenticationStatus` composable so the home page shows a "loading" text while the Nhost client is determining the authentication status of the user. Let's also hide the sign up and sign in links when the user is already authenticated:
