...
const nhost = new NhostClient({
  subdomain: process.env.YOUR_SUBDOMAIN,
...
`YOUR_SUBDOMAIN` and `YOUR_REGION` to store your Nhost domain details:

```yaml title=".env.local"
YOUR_SUBDOMAIN = YOUR_NHOST_SUBDOMAIN
YOUR_REGION = YOUR_NHOST_REGION
```
For localhost, 

```yaml title=".env.local"
YOUR_SUBDOMAIN = localhost
YOUR_REGION = 
```
You can find your Nhost domain details for your project from [your dashboard](https://app.nhost.io) as shown below:
