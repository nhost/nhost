 ...
 */
async function basicPasswordChange(
  ...
  if (await isCompromisedPassword(newPassword)) {
    req.logger.verbose(
      `User ${userId} tried changing his password but it was too weak`,
      {
        userId,
      }
    );
  ...
  req.logger.verbose(
    `User ${userId} directly changed password to ${newPasswordHash}`,
    {
      userId,
      newPasswordHash,
    }
