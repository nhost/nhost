...
async function loginAccount(req: ValidatedRequest<Schema>, res: Response) {
  ...
  if (isMagicLinkLogin(body)) {
    ...
    req.logger.verbose(
      `User ${user.id} logged in with magic link to email ${email}`,
      {
        userId: user.id,
        email,
      }
  ...
  if (
    AUTHENTICATION.USER_IMPERSONATION_ENABLED &&
    hasAdminSecret &&
...
export default (router: Router) => {
  router.post(
    ...
    (req: ValidatedRequest<Schema>, res: Response, next: NextFunction) => {
      if (
        isAnonymousLogin(req.body) &&
        !AUTHENTICATION.ANONYMOUS_USERS_ENABLED
      ) {
        return res.boom.badRequest("Anonymous login is disabled");
      } else if (
        isMagicLinkLogin(req.body) &&
        !AUTHENTICATION.MAGIC_LINK_ENABLED
      ) {
