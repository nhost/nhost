...
it('should fail to activate an user from a wrong ticket', (done) => {
  withEnv(
    ...
    request,
    (done) => {
      request.get(`/activate?ticket=${uuidv4()}`).expect(401).end(end(done))
    },
    done
  )
})
...
it('should not sign the user in without password when magic link is disabled', (done) => {
  withEnv(
    ...
    request,
    (done) => {
      request.post('/login').send({ email: generateRandomEmail() }).expect(400).end(end(done))
    },
    done
  )
})
...
it('should logout', (done) => {
  registerAndLoginAccount(request).then(({ refresh_token }) => {
    request.post(`/logout`).query({ refresh_token }).send().expect(204).end(end(done))
  })
})
