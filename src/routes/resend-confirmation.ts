...
import { asyncWrapper, getUserByEmail, userToSessionUser } from "@/helpers";
...
async function resendConfirmation(
  ...
  if (!user) {
    ...
    +new Date(user.lastConfirmationEmailSentAt) +
      REGISTRATION.CONFIRMATION_RESET_TIMEOUT >
    +new Date()
  ) {
    req.logger.verbose(
  ...
  await gqlSdk.updateUser({
    ...
    user: {
      lastConfirmationEmailSentAt: new Date(
        +Date.now() + REGISTRATION.CONFIRMATION_RESET_TIMEOUT
      ),
      ticket,
      ticketExpiresAt,
    },
  ...
  req.logger.verbose(
    `User ${user.id} requested a confirmation email reset to ${user.email}`,
    {
      userId: user.id,
      email: user.email,
    }
