...
test('should create a table with a composite foreign key', async ({
  authenticatedNhostPage: page,
}) => {
  ...
  // a composite foreign key needs a composite candidate key to point at
  await page.getByRole('button', { name: /add unique constraint/i }).click();
  ...
  // toggle the popover shut so it stops covering the dialog's submit button
  await page.getByRole('combobox', { name: 'Columns' }).click();
  ...
  await page.getByRole('combobox', { name: 'Referenced key' }).click();
  await page
    .getByRole('option', { name: /UNIQUE .*\(region_code, dept_code\)/ })
    .click();
  ...
  // reopening resolves the composite key back out of introspection
  await page
    .locator(
      `li:has-text("${employeeTableName}") #table-management-menu-${employeeTableName}`,
    )
    .click();
  await page.getByText('Edit Table').click();
  await expect(page.locator('h2:has-text("Edit Table")')).toBeVisible();
