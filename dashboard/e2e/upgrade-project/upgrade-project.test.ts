...
test.beforeAll(async ({ browser }) => {
  ...
  console.log('BEFORE ALL upgrade project\n');
  await loginWithFreeUser(page);
});

test.only('create org', async () => {
  ...
  // .fill('123 Main Street, Springfield, IL 62701');
  await stripeFrame
  ...
  await await stripeFrame.getByTestId('hosted-payment-submit-button').click();
  await page.waitForURL(/orgs\/verify/);
  await page.waitForSelector(
    'span:has-text("Processing new organization request")',
  );
...
test('should upgrade the project', async () => {
  ...
  await stripeFrame.getByText('Billing address').scrollIntoViewIfNeeded();
  await stripeFrame
    .locator('#billingCountry')
    .selectOption({ label: 'Germany' });

  const payAndSubscribeButton = await stripeFrame.getByTestId(
    'hosted-payment-submit-button',
  );
  await payAndSubscribeButton.scrollIntoViewIfNeeded();
  await payAndSubscribeButton.click();
