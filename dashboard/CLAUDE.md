...
- Native-query and logical-model CRUD/permissions use their operation-specific metadata and migration helpers. Pass one explicit source in mutation variables (not inside `args`); it must drive platform commands, local migration `datasource`, and every nested `up`/`down` step. Keep atomic CRUD untrack/retrack steps together and preserve ordinary permission bulk ordering. Local writes use CLI `/apis/migrate` with implicit metadata export; allow snapshot reads on `/v1/metadata`, but no explicit `/apis/metadata` export. The CLI drops `resource_version`, so local mutations have no optimistic concurrency guarantee.
- Native queries support PostgreSQL sources only. Reuse cached metadata, scope models/queries/permissions/relationships to the selected source, and never fall back for an invalid explicit source. Creation may switch sources; editing and inline model creation lock source. Source changes clear model references without resetting the draft.
