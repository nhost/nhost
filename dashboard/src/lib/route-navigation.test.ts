...
describe('isRouteActive', () => {
  // Non-exact links match a page scope: the path (equal or segment-boundary
  // prefix) decides, and query strings are ignored. Exact links match one
  // precise destination: the path must be equal and the href's query params
  // must all be present in the URL.
  it.each([
    ['/settings?tab=version', '/settings?tab=version', false, true],
    ['/settings?tab=capacity', '/settings?tab=version', false, true],
    ['/settings', '/settings?tab=version', false, true],
    ...
    ['/settings?tab=capacity', '/settings?tab=version', true, false],
    ['/settings', '/settings?tab=version', true, false],
    ['/settings?tab=', '/settings?tab=version', true, false],
    ['/settings?tab=capacity&marker=foo', '/settings?tab=capacity', true, true],
    [
    ...
    ['/settings?tab=capacity', '/settings', true, true],
    [
    ...
    ['/settings?tab=Capacity', '/settings?tab=capacity', true, false],
    [
    ...
    ['/settings?tab=cap%61city', '/settings?tab=capacity', true, true],
    [
