...
import useActivateMfa from '@/features/account/settings/components/AccountMfaSettings/hooks/useActivateMfa';
...
function MfaQRCodeAndTOTPSecret({ onSuccess }: Props) {
  const {
    ...
    isGenerated,
    isGenerating,
    activateMfa: activateMfaFn,
    isActivating,
    totpSecret,
  } = useConfigMfa();
  const activateMfa = useActivateMfa({ activateMfaFn, onSuccess });
