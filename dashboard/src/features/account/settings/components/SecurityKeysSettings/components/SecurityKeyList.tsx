...
import RemoveSecurityKey from './RemoveSecurityKey';
...
function SecurityKey({ id, nickname }: SecurityKeyProps) {
  return (
    <div className="flex w-full items-center justify-between rounded-lg border border-[#EAEDF0] px-2 py-2 font-['Inter_var'] dark:border-[#2F363D]">
      <div className="flex justify-start gap-3">
        <Fingerprint />
        <span>{nickname || id}</span>
      </div>
      <RemoveSecurityKey id={id} />
    </div>
  );
}
  ...
  return (
    <div>
      ...
      {!loading && data?.authUserSecurityKeys.length === 0 && (
        <InfoAlert>No security key have been added yet!</InfoAlert>
      )}
