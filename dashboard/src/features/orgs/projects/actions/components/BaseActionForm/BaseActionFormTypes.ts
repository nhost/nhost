...
} from '@/features/orgs/projects/actions/utils/constants';
...
export const validationSchema = z.object({
  actionDefinitionSdl: z
    ...
    .superRefine((sdl, ctx) => {
      const { error } = parseActionDefinitionSdl(sdl);
      if (error !== null) {
...
> = {
  sampleInput: getActionSampleInputPayload(
    parseActionDefinitionSdl(DEFAULT_ACTION_DEFINITION_SDL).definition ??
      undefined,
  ),
