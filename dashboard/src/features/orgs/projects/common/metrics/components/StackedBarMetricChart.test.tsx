...
const POINTER_COORDS = { clientX: 320, clientY: 100 };
...
describe('StackedBarMetricChart', () => {
  ...
  it('renders legend addons next to, not inside, the series toggle', () => {
    render(
      <StackedBarMetricChart
        data={[
          series('compute', 'Compute', [100, 120, 90]),
        ...
        accessors={accessors}
        legendItemAddon={(key) =>
          key === 'plan' ? <button type="button">About plan</button> : null
        }
      />,
    );

    const addon = screen.getByRole('button', { name: 'About plan' });
    const toggle = screen.getByRole('button', { name: 'Plan' });
    expect(toggle).not.toContainElement(addon);
    expect(toggle.parentElement).toContainElement(addon);
    expect(toggle.parentElement).not.toContainElement(
  ...
  it('uses the primary line color for included usage in the tooltip', async () => {
    ...
    await user.pointer({
      target: container.querySelector('.recharts-wrapper') as Element,
      coords: POINTER_COORDS,
    });
  ...
  it('layers the hover tooltip above the legend', () => {
    const { container } = render(
      <StackedBarMetricChart
        data={[series('egress', 'Egress', [100, 120, 190])]}
        accessors={accessors}
  ...
  it('pins a selectable tooltip on click and closes it explicitly', async () => {
    ...
    await user.pointer({ target: chart, coords: POINTER_COORDS });
    await waitFor(() => {
