...
export interface UseEventTriggerPaginationOptions<TArgs, TData = unknown[]> {
   ...
   * Pass an identity of the listing (e.g., data source + trigger name).
   */
  resetKey?: unknown;
}

export interface UseEventTriggerPaginationResult<TData = unknown[]> {
  offset: number;
  limit: number;
...
export default function useEventTriggerPagination<TArgs, TData = unknown[]>({
  ...
  getPageLength,
  resetKey,
}: UseEventTriggerPaginationOptions<
  TArgs,
  TData
>): UseEventTriggerPaginationResult<TData> {
