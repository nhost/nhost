...
describe('LogicalModelCustomCheckEditor', () => {
  ...
  it('marks an empty NOT invalid and recovers when deleting it without changing the saved filter', async () => {
    const onChange = vi.fn();
    ...
    await user.click(await screen.findByText('not', { exact: true }));
    await waitFor(() =>
  ...
  it.each([
    'and',
    'or',
  ])('rejects an empty %s group without committing it', async (operator) => {
    const onChange = vi.fn();
    const onValidityChange = vi.fn();
    const user = new TestUserEvent();
    render(
