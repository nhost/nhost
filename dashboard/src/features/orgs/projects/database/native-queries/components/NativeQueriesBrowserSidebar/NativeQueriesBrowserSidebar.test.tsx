...
describe('NativeQueriesBrowserSidebar', () => {
  ...
  afterEach(() => {
    ...
    for (const body of mutationBodies) {
      expect(body.name).toMatch(
        /^(track|update|untrack)_(logical_model|native_query)_/,
      );
      expect(body.datasource).toBe('default');
      expect(body.up[0].type).toBe('bulk_atomic');
      expect(body.up.slice(1)).toEqual(
        body.up.length === 1
          ? []
          : [
              {
  ...
  it.each([
    'edit native query',
    'edit logical model',
  ] as const)('keeps a failed %s save dirty and retries the preserved draft', async (surface) => {
    server.use(
      migrationHandler(() =>
  ...
  it.each([
    'edit native query',
    'edit logical model',
  ] as const)('keeps the %s drawer open until the migration completes', async (surface) => {
    ...
    try {
      await user.clear(description);
      await user.type(description, 'Awaited draft');
      await user.click(save);
      await waitFor(() => expect(mutationBodies).toHaveLength(1));
      expect(description).toHaveValue('Awaited draft');
  ...
  it('opens edit from the item menu and submits the existing model', async () => {
    ...
    expect(mutationBodies[0]).toMatchObject({
      name: 'update_logical_model_author_result',
      datasource: 'default',
      up: [
        {
          type: 'bulk_atomic',
    ...
    expect(mutationBodies[0]?.down?.[0]).toMatchObject({
      type: 'bulk_atomic',
      args: [
        {
          type: 'pg_untrack_logical_model',
    ...
    expect(mutationBodies[0]?.down?.at(-1)).toEqual({
      type: 'pg_create_logical_model_select_permission',
      args: {
        source: 'default',
        name: 'author_result',
