...
const nativeQueryBaseSchema = z.object({
...
export const createNativeQueryFormSchema = (
  ...
  nativeQueryBaseSchema.superRefine((values, context) => {
    ...
    if (
      values.returns &&
      logicalModelNames &&
      !logicalModelNames.includes(values.returns)
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['returns'],
...
interface NativeQueryFormSubmissionContext {
...
export interface BaseNativeQueryFormProps {
  ...
  onSubmit: (
    values: NativeQueryFormValues,
    context: NativeQueryFormSubmissionContext,
  ) => Promise<void> | void;
