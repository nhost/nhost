...
describe('CreateLogicalModelForm', () => {
  ...
  it('matches native-query drawer chrome for create and edit forms', () => {
    const { rerender } = render(<CreateLogicalModelForm />);
  ...
  it('ignores a successful create response after the draft unmounts', async () => {
    ...
    const onCancel = vi.fn();
    const user = new TestUserEvent();
    const view = render(<CreateLogicalModelForm onCancel={onCancel} />);

    await fillLogicalModel(user, 'late_result');
    await user.click(screen.getByRole('button', { name: 'Create' }));
    await waitFor(() => expect(mocks.mutateAsync).toHaveBeenCalledOnce());
    view.unmount();
    await act(async () => {
  ...
  it('keeps a newer same-route draft when an earlier create response succeeds', async () => {
    ...
    expect(() => act(() => mocks.routeChangeStart?.())).toThrow(
      'Unsaved changes',
    );
  });
