...
describe('CreateNativeQueryForm', () => {
  ...
  it('ignores a successful create response after the draft unmounts', async () => {
    mocks.modelsResult.data = [{ name: 'author_result' }];
    const completion = Promise.withResolvers<{ message: string }>();
    mocks.nativeMutateAsync.mockReturnValueOnce(completion.promise);
    const onCancel = vi.fn();
    const user = new TestUserEvent();
    const view = render(<CreateNativeQueryForm onCancel={onCancel} />);
  ...
  it('ignores a successful create response after navigation changes the draft route', async () => {
    mocks.modelsResult.data = [{ name: 'author_result' }];
    const completion = Promise.withResolvers<{ message: string }>();
    mocks.nativeMutateAsync.mockReturnValueOnce(completion.promise);
    const onCancel = vi.fn();
    ...
    await user.click(screen.getByRole('button', { name: 'Create' }));
    await waitFor(() => expect(mocks.nativeMutateAsync).toHaveBeenCalledOnce());
    mocks.router.asPath = '/orgs/test-org/projects/test-app/database';
    await act(async () => {
      completion.resolve({ message: 'success' });
      await completion.promise;
    });
  ...
  it('keeps a newer same-route draft when an earlier create response succeeds', async () => {
    mocks.modelsResult.data = [{ name: 'author_result' }];
    const completion = Promise.withResolvers<{ message: string }>();
    mocks.nativeMutateAsync.mockReturnValueOnce(completion.promise);
    const onCancel = vi.fn();
    ...
    expectNativeQueryDraft('SELECT * FROM authors LIMIT 10');
    expect(mocks.router.push).not.toHaveBeenCalled();
    expect(onCancel).not.toHaveBeenCalled();
  });
  ...
  it('maps a trimmed create description to metadata description', async () => {
    mocks.modelsResult.data = [{ name: 'author_result' }];
    const user = new TestUserEvent();
    render(<CreateNativeQueryForm />);
    ...
    await waitFor(() => expect(mocks.nativeMutateAsync).toHaveBeenCalledOnce());
    const submitted = mocks.nativeMutateAsync.mock.calls[0]?.[0];
    expect(submitted?.source).toBe('default');
    expect(submitted?.args).toEqual({
      ...
      code: 'SELECT 1',
      returns: 'author_result',
      description: 'Public author search',
    });
    expect(submitted?.args).not.toHaveProperty('comment');
  });
