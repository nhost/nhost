...
function NativeQueryDetailsContent({ source }: { source: string }) {
  ...
  const modelHref = `/orgs/${orgSlug}/projects/${appSubdomain}/database/native-queries/${encodeURIComponent(source)}/models/${encodeURIComponent(query.returns)}`;
      ...
      <NativeQueriesDetailsHeader
        ...
        onEdit={() =>
          openDrawer({
            ...
            component: <EditNativeQueryForm query={query} source={source} />,
          })
        }
      />
...
export default function NativeQueryDetails() {
  ...
  return <NativeQueryDetailsContent source={source} />;
}
