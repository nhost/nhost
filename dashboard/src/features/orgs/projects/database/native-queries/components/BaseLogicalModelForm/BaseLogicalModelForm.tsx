...
function validateLogicalModelReference(
  ...
  if (type.kind === 'logical_model') {
    if (type.logicalModel && !logicalModelNames.includes(type.logicalModel)) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: [...path, 'logicalModel'],
        message: 'The selected logical model is unavailable.',
      });
    }
    return;
  }
...
export const createLogicalModelFormSchema = (
  ...
  logicalModelBaseSchema.superRefine((values, context) => {
    if (!sourceOptions.includes(values.source)) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['source'],
        message: 'The selected data source is unavailable.',
      });
    }
...
export const clearLogicalModelReferences = (
  type: LogicalModelTypeNode,
): LogicalModelTypeNode => {
  if (type.kind === 'logical_model') {
    return { ...type, logicalModel: '' };
  }

  if (type.kind === 'array') {
    return { ...type, item: clearLogicalModelReferences(type.item) };
  }
...
type FormErrorValue = Record<string, unknown> | string;

function getErrorProperty(
  error: unknown,
  property: string,
): FormErrorValue | undefined {
...
interface LogicalModelFormSubmissionContext {
...
export interface BaseLogicalModelFormProps {
  ...
  onSubmit: (
    values: LogicalModelFormValues,
    context: LogicalModelFormSubmissionContext,
  ) => Promise<void> | void;
...
export default function BaseLogicalModelForm({
  ...
  const validationOptionsKey = useMemo(
    () => JSON.stringify([sourceOptions, existingNames, logicalModelNames]),
    [existingNames, logicalModelNames, sourceOptions],
  );
            ...
            render={({ field }) => (
              <Select
                ...
                onValueChange={(source) => {
                  ...
                  form.setValue(
                    'fields',
                    form.getValues('fields').map((modelField) => ({
                      ...modelField,
                      type: clearLogicalModelReferences(modelField.type),
                    })),
                    { shouldDirty: true, shouldValidate: true },
                  );
