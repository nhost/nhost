...
// reset may replace an already-mounted draft or clear its validation error, so
// the parent remounts this editor by key instead of pushing the value in.
    ...
    <div>
      <Textarea
        ...
        onChange={(event) => {
          ...
          if (hasEmptyBooleanGroup(parsed)) {
            setError(
              'Empty _and, _or, and _not groups match every row. Add a condition or remove the group.',
            );
            onValidityChange(false);
            return;
          }
...
export function LogicalModelCustomCheckEditor({
  ...
  const lastCommittedRaw = useRef<Record<string, unknown> | undefined>(
    undefined,
  );
  if (lastCommittedRaw.current === undefined) {
  ...
  const [jsonResetGeneration, setJsonResetGeneration] = useState(0);
  ...
  useEffect(() => {
    ...
    const subscription = form.watch((_formValue, info) => {
      ...
      queueMicrotask(() => {
        ...
        if (!result.success) {
          setVisualError(result.errors.map(({ message }) => message).join(' '));
          validityRef.current?.(false);
          return;
        }
        setVisualError(undefined);
        validityRef.current?.(true);
        if (dequal(result.value, lastCommittedRaw.current)) {
          return;
        }
  ...
  useEffect(() => {
    if (dequal(value, lastCommittedRaw.current)) {
      return;
    }
    ...
    setVisualError(undefined);
    setJsonResetGeneration((generation) => generation + 1);
    validityRef.current?.(true);
  ...
  useEffect(() => {
    ...
    resetGeneration.current += 1;
    form.reset({ filter: compatibility.node });
    setVisualError(undefined);
    validityRef.current?.(true);
  }, [compatibility, form, mode]);

  function handleJsonChange(nextValue: Record<string, unknown>) {
    if (dequal(nextValue, lastCommittedRaw.current)) {
      return;
    }
