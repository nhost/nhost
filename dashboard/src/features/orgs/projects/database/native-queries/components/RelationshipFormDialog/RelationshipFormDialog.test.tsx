...
describe('RelationshipFormDialog', () => {
  ...
  describe.each([
    'object',
    'array',
  ] as const)('editing an %s relationship', (kind) => {
    it.each([
      undefined,
      null,
      'before_parent',
      'after_parent',
    ] as const)('submits insertion order %s with the omission default', async (insertionOrder) => {
      const onSubmit = vi.fn().mockResolvedValue(undefined);
      ...
      const originalQuery: NativeQueryItem = {
        ...query,
        object_relationships: kind === 'object' ? [originalRelationship] : [],
