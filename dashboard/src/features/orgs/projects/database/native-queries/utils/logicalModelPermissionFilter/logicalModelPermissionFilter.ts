...
function serializeRuleNode(
  ...
  selectablePaths: ReadonlySet<string>,
  path: string,
  allowEmptyBooleanGroups: boolean,
  ...
  if (isInvalidNode(node)) {
    return { success: true, value: serializeNode(node) };
  }
  ...
  if (node.operator === '_and' || node.operator === '_or') {
    if (!allowEmptyBooleanGroups && childValues.length === 0) {
      return {
        success: false,
        errors: [
          error(
            'malformed-boolean-group',
            path,
            `${node.operator} must contain at least one child.`,
          ),
        ],
      };
    }
...
export function serializeLogicalModelFilter(
  ...
  return serializeRuleNode(node, new Set(fields.selectablePaths), '$', false);
}
...
export function analyzeLogicalModelFilter(
  ...
  // Existing empty groups must remain lossless for display; visual authoring rejects them.
  const serialized = serializeRuleNode(
    parsed.node,
    new Set(fields.selectablePaths),
    '$',
    true,
  );
...
// An empty Boolean group evaluates to true, so authoring one silently grants
// unrestricted row access. Comparison operands are data, not
// expressions, so their contents are never inspected.
export function hasEmptyBooleanGroup(value: unknown): boolean {
  if (!isPlainObject(value)) {
    return false;
  }
  return Object.entries(value).some(([key, child]) => {
    if (key === '_and' || key === '_or') {
      ...
      return child.some(
        (item) => isEmptyExpression(item) || hasEmptyBooleanGroup(item),
      );
    }
    if (key === '_not') {
      return isEmptyExpression(child) || hasEmptyBooleanGroup(child);
    }
    if (key.startsWith('_')) {
      return false;
    }
    return hasEmptyBooleanGroup(child);
  });
}
