...
function semanticDeepEqual(left: unknown, right: unknown): boolean {
  ...
  if (Array.isArray(left) || Array.isArray(right)) {
    ...
    return left.every((item, index) => semanticDeepEqual(item, right[index]));
  }
  ...
  return leftKeys.every(
    (key) =>
      Object.hasOwn(right, key) && semanticDeepEqual(left[key], right[key]),
  );
}
...
export function analyzeLogicalModelFilter(
  ...
  if (!semanticDeepEqual(filter, serialized.value)) {
    return {
      compatible: false,
      errors: [
        error(
