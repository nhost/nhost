...
describe('resolveLogicalModelFieldDescriptors', () => {
  ...
  it('defaults omitted scalar and reference nullability while preserving explicit values', () => {
    const model: LogicalModelItem = {
      name: 'author',
      fields: [
        { name: 'id', type: { scalar: 'uuid' } },
        { name: 'profile', type: { logical_model: 'profile' } },
      ],
    };
    const nestedModel: LogicalModelItem = {
      name: 'profile',
      fields: [
    ...
    expect(fields(model, [model, nestedModel])).toMatchObject({
      descriptors: [
        { path: 'id', nullable: false },
        { path: 'profile', nullable: false },
        { path: 'profile.active', nullable: false },
        { path: 'profile.displayName', nullable: true },
      ],
      selectablePaths: ['id', 'profile.active', 'profile.displayName'],
      traversalPaths: ['profile'],
      issues: [],
...
describe('serializeLogicalModelFilter', () => {
  it.each([
    '_and',
    '_or',
  ] as const)('rejects empty %s authoring but preserves stored filters for display', (operator) => {
