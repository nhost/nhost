...
describe('fetchTableSchema', () => {
  ...
  it('omits composite and malformed relations while preserving singular relations', async () => {
    fetchMock.mockResolvedValueOnce(mixedSchemaResponse());
  ...
  it('uses the materialized-view column query', async () => {
    fetchMock.mockResolvedValueOnce(ok([queryResult([]), queryResult([])]));
