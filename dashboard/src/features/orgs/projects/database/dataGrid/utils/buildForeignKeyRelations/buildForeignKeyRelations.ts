...
/**
 * A row returned by `CONSTRAINT_DEFINITION_QUERY`. Declared as a type alias
 * rather than an interface so it keeps an implicit index signature and stays
 * assignable from parsed JSON without a double assertion.
 */
export type RawTableConstraint = {
