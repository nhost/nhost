...
function resolveConstraintName(constraint: FormUniqueConstraint): string {
  if (constraint.originalName && constraint.name === constraint.originalName) {
    return constraint.originalName;
  }
...
export default function serializeFormUniqueConstraints(
  ...
  return formUniqueConstraints.map((constraint) => ({
    ...
    name: resolveConstraintName(constraint),
