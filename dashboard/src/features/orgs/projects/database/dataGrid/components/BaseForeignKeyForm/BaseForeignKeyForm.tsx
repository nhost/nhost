...
export default function BaseForeignKeyForm({
  ...
  // Candidate ids are JSON tuples, so they can never collide with this sentinel.
  const isUnmanagedTarget = referencedKeyId === 'unmanaged';
  ...
  return (
    <Form
      onSubmit={(values) => {
        ...
        if (
          values.referencedKeyId !== 'unmanaged' &&
          (!selectedCandidate ||
      ...
      <div className="grid flex-auto grid-flow-row gap-4 overflow-y-auto border-t-1 py-4">
        <section className="grid grid-flow-row gap-4 px-6">
          ...
          <ReferencedKeySelect
            ...
            unmanagedLabel={isUnmanagedTarget ? unmanagedLabel : undefined}
