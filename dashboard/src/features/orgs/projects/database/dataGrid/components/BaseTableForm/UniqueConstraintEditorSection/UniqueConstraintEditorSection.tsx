...
function getConstraintErrorMessage(
  error: FieldErrors['uniqueConstraints'],
): string | undefined {
...
function ConstraintErrorMessage() {
  ...
  const message = getConstraintErrorMessage(errors.uniqueConstraints);
    ...
    <div className="grid grid-flow-row gap-2 px-6">
      {fields.map((field, constraintIndex) => (
        <UniqueConstraintEditorRow
          ...
          onEdit={() => {
            const constraint = getValues(
              `uniqueConstraints.${constraintIndex}`,
