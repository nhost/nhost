    ...
    <div className="flex h-full flex-col justify-between">
      <div className="box flex flex-col px-2">
        {schemas && schemas.length > 0 && (
          <Select value={selectedSchema} onValueChange={setSelectedSchema}>
            ...
            <SelectContent>
              {schemas.map((schema) => (
                <SelectItem
                  className="data-[state=checked]:bg-accent dark:hover:bg-muted dark:data-[state=checked]:bg-muted"
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    {(isSchemaLocked(schema.schema_name) ||
                      isGitHubConnected) && (
                      <Lock className="h-3 w-3 text-[#556378] dark:text-[#a2b3be]" />
                    )}
                  </div>
                </SelectItem>
              ))}
        ...
        {isGitHubConnected && (
          <div className="box mt-1.5 flex items-center gap-1 px-2">
            <Info className="text-disabled h-4 w-4" />
        ...
        {!isSelectedSchemaLocked && (
          <Button
            variant="link"
            className="hover:bg-light-action-hover disabled:text-disabled mt-1 flex w-full justify-between px-[0.625rem] !text-sm+ text-primary hover:no-underline hover:dark:bg-theme-grey-200"
            onClick={() => {
              openDrawer({
        ...
        {isNotEmptyValue(schemas) && isEmptyValue(tablesInSelectedSchema) && (
          <p className="text-disabled px-2 py-1.5 text-xs">No tables found.</p>
        )}
                ...
                return (
                  <li className="group pb-1" key={tablePath}>
                    <Button
                      ...
                      className={cn(
                        'hover:bg-light-action-hover flex w-full max-w-full justify-between pl-0 text-sm+ hover:no-underline hover:dark:bg-theme-grey-200',
                        {
      ...
      <div className="box border-t">
        <Button
          ...
          asChild
          className={cn(
            'hover:bg-light-action-hover flex rounded-none border text-sm+ hover:no-underline group-focus-within:pr-9 group-hover:pr-9 group-active:pr-9 hover:dark:bg-theme-grey-200',
