...
export default function EditRemoteRelationshipButton({
  ...
  const toSourceDefinition = isToSourceRelationshipDefinition(
    relationshipDefinition,
  )
    ? relationshipDefinition.to_source
    : null;
  ...
  const initialValues = useMemo<BaseRelationshipFormValues | null>(() => {
    if (!relationshipDefinition || !toSourceDefinition) {
      return null;
    }
    ...
    return {
      ...
      toReference: {
        ...
        source: ReferenceSource.createTypeSourceFromName(
          toSourceDefinition.source ?? '',
        ).fullValue,
      },
      relationshipType:
  ...
  if (!relationshipDefinition || !toSourceDefinition || !initialValues) {
    return null;
  }
