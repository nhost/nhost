...
function compareLoadedFiltersToStorage(tablePath: string, filters: DataGridFilter[]) {
...
const DataGridQueryParamsContext = createContext<DataGridQueryParamsContextProps>({
...
function DataGridQueryParamsProvider({ children }: PropsWithChildren) {
  ...
  const [currentOffset, setCurrentOffset] = useState<number>(parseInt(page as string, 10) - 1 || 0)
  ...
  useEffect(() => {
    const filtersForTheTable = PersistenDataGrdiFilterStorage.getDataGridFilters(tablePath)
    _setAppliedFilters(filtersForTheTable)
    setSortBy([])
  }, [tablePath])

  isFiltersLoadedFromStorage.current = compareLoadedFiltersToStorage(tablePath, appliedFilters)
