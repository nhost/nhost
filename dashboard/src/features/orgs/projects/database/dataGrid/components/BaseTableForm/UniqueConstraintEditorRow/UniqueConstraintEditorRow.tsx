...
export default function UniqueConstraintEditorRow({
  ...
  // Renders once after a constraint is removed but before the list re-renders.
  const constraint: FormUniqueConstraint | undefined = useWatch({
    control,
    name: `uniqueConstraints.${index}`,
  });
  const columns = useWatch({ control, name: 'columns' }) ?? [];
  const tableName = useWatch({ control, name: 'name' }) ?? '';
