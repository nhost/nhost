...
describe('prepareTrackForeignKeyRelationsMetadata', () => {
  ...
  it.each([
    ...
    'creates the missing self-referencing side for a %s relation with only the %s side tracked',
    async (_cardinality, existingSide, oneToOne) => {
  ...
  it.each([['one-to-many', 'one-to-one', false, true]] as const)(
    'keeps a %s relation tracked after its cardinality changes to %s',
    async (_trackedCardinality, _currentCardinality, trackedOneToOne, oneToOne) => {
      const reverseConstraint = {
        column: 'team_id',
