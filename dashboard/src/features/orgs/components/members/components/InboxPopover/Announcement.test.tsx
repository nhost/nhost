...
describe('Announcement', () => {
  ...
  describe.each([
    ...
    it.each(['GraphQL', 'network'] as const)(
      'shows an error toast on a %s failure',
      async (failure) => {
    ...
    it('updates the read state without showing a toast on success', async () => {
      ...
      const remainingAction = state === 'read' ? 'unread' : 'read';

      await waitFor(() => {
        expect(
          screen.getByRole('menuitem', { name: `Mark as ${remainingAction}` }),
        ).toBeInTheDocument();
      });
      expect(
        screen.queryByRole('menuitem', { name: `Mark as ${state}` }),
