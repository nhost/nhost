...
describe('BillingTabs', () => {
  ...
  it('redirects a free organization off a deep-linked restricted tab without mounting its panel', async () => {
  ...
  it('disables the paid-only tabs for a free organization and explains why on hover', async () => {
    setRouter({ orgSlug: 'xyz' });
    server.use(organizationHandler({ isFree: true }));
    const user = new TestUserEvent();
    render(<BillingTabs />);
    ...
    expect(screen.getByRole('tab', { name: 'Invoices' })).toBeDisabled();
    expect(screen.getByRole('tab', { name: 'Plan' })).toBeEnabled();
    expect(
      await screen.findByRole('tabpanel', { name: 'Plan' }),
    ).toBeInTheDocument();

    // The trigger itself is pointer-events-none while disabled, so the cursor
    // has to come from the wrapper that actually receives the hover.
    ['Usage', 'Invoices'].forEach((name) => {
      expect(screen.getByRole('tab', { name }).parentElement).toHaveClass(
        'cursor-not-allowed',
      );
    });

    await user.hover(screen.getByRole('tab', { name: 'Usage' }));

    expect(
      await screen.findByRole('tooltip', { name: 'Available on paid plans' }),
    ).toBeInTheDocument();
  });
