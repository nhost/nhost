...
describe('TransferProjectDialog', () => {
  ...
  test('opens create org dialog when selecting "create new org" and closes transfer dialog', async () => {
    ...
    await waitFor(() => {
      expect(submitButtonAfterClosingNewOrgDialog).toHaveTextContent(
        'Continue',
      );
    });
  });
  ...
  test(`transfer dialog opens automatically when there is a session_id and selects the ${newOrg.name} from the dropdown`, async () => {
    ...
    const processingNewOrgText = await screen.findByTestId('message');

    expect(processingNewOrgText).toHaveTextContent('Creating new organization');
