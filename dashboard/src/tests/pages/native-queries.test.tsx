...
describe('native-query source routes', () => {
  ...
  it('uses the default overview only when a default source exists', async () => {
  ...
  it('shows empty and metadata-error states without redirecting', async () => {
    metadata.metadata.sources = [];
    const view = render(<NativeQueriesLandingPage />);
    await screen.findByRole('heading', { name: 'No data sources' });
    metadataStatus = 500;
    await act(async () => {
  ...
  it('never falls back from an explicit unknown source URL', async () => {
    ...
    expect(
      screen.queryByRole('link', { name: 'shared_model' }),
