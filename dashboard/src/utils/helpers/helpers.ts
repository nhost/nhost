...
/**
 * Gets the different tables, functions and views if there's any
 * from the sql query
 * @param sqlQuery The sql query
 */
export function extractEntitiesFromSQL(sqlQuery: string): ParsedSQLEntity[] {
  const tableRegex = /CREATE TABLE\s+((\w+)\.)?(\w+)/gi;
  const functionRegex = /CREATE FUNCTION\s+((\w+)\.)?(\w+)/gi;
  const viewRegex = /CREATE VIEW\s+((\w+)\.)?(\w+)/gi;
  ...
  const tableMatches = parseMatches(
    Array.from(sqlQuery.matchAll(tableRegex)),
    'table',
  );

  const functionMatches = parseMatches(
    Array.from(sqlQuery.matchAll(functionRegex)),
    'function',
  );

  const viewMatches = parseMatches(
    Array.from(sqlQuery.matchAll(viewRegex)),
    'view',
  );

  return [...tableMatches, ...functionMatches, ...viewMatches];
}
