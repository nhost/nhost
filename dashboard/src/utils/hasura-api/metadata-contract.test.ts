...
const atomicTrackingOperation: NativeQueryMetadataBulkOperation = {
  type: 'bulk_atomic',
  resource_version: 42,
  args: [
    {
...
const bulkPermissionOperation: NativeQueryMetadataBulkOperation = {
  type: 'bulk',
  resource_version: 43,
  args: [
    {
...
describe('native query and logical model metadata contract', () => {
  ...
  it('serializes track/untrack steps with bulk_atomic through the metadata client', async () => {
    const response = await metadataOperation(atomicTrackingOperation, {
      appUrl: METADATA_BASE_URL,
    });

    expect(response.status).toBe(200);
    expect(capturedBody).toEqual(atomicTrackingOperation);
  });

  it('serializes logical model permission steps with bulk through the metadata client', async () => {
    const response = await metadataOperation(bulkPermissionOperation, {
      appUrl: METADATA_BASE_URL,
    });

    expect(response.status).toBe(200);
    expect(capturedBody).toEqual(bulkPermissionOperation);
  });
