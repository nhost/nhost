...
const stubNhostProxy = (
  ...
  vi.mocked(handleNhostProxy).mockImplementation(async (request) => {
    ...
    return {
      session: value,
      clearLinkParams: mutations.clearLinkParams ?? false,
...
describe('proxy auth email links', () => {
  it('redirects to the same page without the code once it is redeemed', async () => {
    stubNhostProxy(session, { clearLinkParams: true });

    const response = await proxy(
      new NextRequest(
        'http://localhost:3000/reset-password?code=abc&type=passwordReset',
      ),
    );
  ...
  it('keeps unrelated query parameters on that redirect', async () => {
    ...
    const response = await proxy(
      new NextRequest('http://localhost:3000/profile?code=abc&tab=email'),
    );
  ...
  it('does not redirect when there was no code to redeem', async () => {
    stubNhostProxy(session);
