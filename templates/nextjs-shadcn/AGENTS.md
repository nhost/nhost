...
- `changePassword` decides for itself whether re-authentication is needed: it reads `hasPassword` from the backend and requires `currentPassword` when the account has one, unless a password-reset grant names this same account. Never gate a sensitive action on an argument the client chooses to send.
- The grant cookie is `httpOnly`, carries the id of the account the redeemed link signed in, and is only honoured for that account; it is cleared on sign-out and on every sign-in. Be precise about what it proves: that this browser redeemed a reset link **for this account**, not that anyone controls its mailbox. Because the session cookie is readable by JS, a holder of a live refresh token can mint one for whatever account that token names, without any email. Treat that as the escalation it is: a stolen session cannot otherwise change the password, because `changePassword` demands the current one, so the grant path is what turns a stolen session into a permanent takeover — changing the password revokes every other refresh token and locks the real owner out. Closing it needs the auth service's PKCE flow, whose `code` is bound to the ticket.
