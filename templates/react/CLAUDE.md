...
- `frontend/` is a Vite single-page app (React 19, TypeScript, React Router) using `@nhost/nhost-js` and TanStack Query. It is client-side only, with no server-side rendering.
...
- `frontend/src/pages/Protected.tsx` defines typed `GetTodos` and `CreateTodo` operations, sends them through `@nhost/nhost-js`, and manages request state with TanStack Query.
...
- Access the Nhost client and session through `useAuth()` from `src/lib/nhost/AuthProvider`. Do not create a second client.
- The browser SDK persists the session in localStorage by default and syncs it across tabs via `sessionStorage.onChange`.
- Protected pages read the session from `useAuth()` and redirect to `/signin` when unauthenticated.
...
- Import generated types through `@/gql`; import app modules with relative paths under `src/`.
...
- `pnpm lint`, `pnpm format`, `pnpm typecheck`, and `pnpm build` validate the app.
