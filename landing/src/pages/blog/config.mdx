...
Today, we're introducing a new mechanism for managing and configuring your projects. This new workflow helps you ensure consistency between local and cloud instances making environment-specific issues a thing of the past. It also gives you more fine-grained control over a project's runtime configuration. It's a significant leap forward in Nhost's commitment in making development easier and more efficient.
...
The new configuration file is supported from `v1.0.0-beta*` onwards, so make sure you are running on the latest CLI. All instructions can be found on the [Migrating to Nhost Config](https://docs.nhost.io/cli/migrate-config) guide. 
...
Let's imagine we want to configure Apple as an oauth provider. We would enable it in our configuration file as follows:
...
We can use the CLI to test and validate that the configuration is  valid:
