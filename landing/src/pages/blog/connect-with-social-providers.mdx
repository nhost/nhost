...
The first thing you have to do, is enable the authentication provider you want your users to be able to connect to. For example, in the screenshot below we enable sign-in with GitHub which you can find when you click on [**Sign-In Methods**](https://app.nhost.io/_/_/settings/sign-in-methods).
...
To verify that the provider was linked to the user's account we just need to query the `user_providers` table using the `useQuery` hook exported from `@apollo/client`. PS. make sure to install the apollo client package by running `npm install @apollo/client`.
