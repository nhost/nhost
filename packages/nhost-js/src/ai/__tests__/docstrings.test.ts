...
import type { AgentEvent, AgentHistoryMessage } from '../index';
...
function requestOptions(callIndex: number): RequestInit {
  ...
  if (!call) {
    throw new Error(`Request ${callIndex} was not made`);
  }
  return call[1] ?? {};
}
  ...
  mockFetch
    ...
    .mockImplementationOnce(() =>
      Promise.resolve(
        jsonResponse({
          data: {
            insertAiAgentSession: { id: 'session-123' },
          },
        }),
      ),
    )
    ...
    .mockImplementationOnce(() =>
      Promise.resolve(
        sseResponse([
          { event: 'content_delta', data: 'Hello ' },
          { event: 'content_delta', data: 'world!' },
          {
            event: 'tool_call',
...
test('approval', async () => {
  mockFetch
    ...
    .mockImplementationOnce(() =>
      Promise.resolve(
        jsonResponse({
          data: { insertAiAgentSession: { id: 'session-abc' } },
        }),
      ),
    )
  ...
  expect(parseRequestBody(2)).toEqual({
...
test('resume', async () => {
  mockFetch
    ...
    .mockImplementationOnce(() =>
      Promise.resolve(
        sseResponse([
          { event: 'content_delta', data: 'Anything else?' },
          { event: 'done', data: '' },
        ]),
      ),
    );
  ...
  test('approve(ids) approves only listed and denies the rest', async () => {
    mockFetch
      .mockImplementationOnce(() =>
        Promise.resolve(
          jsonResponse({
            data: { insertAiAgentSession: { id: 's2' } },
          }),
        ),
      )
      .mockImplementationOnce(() =>
          ...
          sseResponse([
            {
              ...
              data: {
                tool_calls: [
                  { id: 'a', name: 'x', requires_approval: true },
                  { id: 'b', name: 'y', requires_approval: false },
                ],
              },
            },
          ]),
...
test('uses the current AI Hasura schema names and message ordering', async () => {
  mockFetch
    .mockImplementationOnce(() =>
      Promise.resolve(
        jsonResponse({ data: { insertAiAgentSession: { id: 'new-session' } } }),
      ),
    )
    .mockImplementationOnce(() =>
      Promise.resolve(
  ...
  const nhost = createClient({ subdomain: 'local', region: 'local' });
  await nhost.ai.newAgentSession({ agentID: 'agent-id' });
  await nhost.ai.resumeSession({ sessionID: 'new-session' });

  const insertBody = parseRequestBody(0) as { query: string };
  expect(insertBody.query).toContain('aiAgentSessions_insert_input!');
  expect(insertBody.query).toContain('insertAiAgentSession');

  const resumeBody = parseRequestBody(1) as { query: string };
  expect(resumeBody.query).toContain('aiAgentSession(id: $sessionID)');
  expect(resumeBody.query).toContain('order_by: { createdAt: asc, seq: asc }');
});
