...
test('error handling for graphql', async () => {
  ...
  try {
    ...
    expect(errors[0]?.message).toBe("field 'restrictedObject' not found in type: 'query_root'")
    expect(error.message).toBe("field 'restrictedObject' not found in type: 'query_root'")
