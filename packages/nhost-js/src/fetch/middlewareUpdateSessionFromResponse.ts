 ...
 */
export const updateSessionFromResponseMiddleware = (storage: SessionStorage): ChainFunction => {
  /**
   ...
   */
  const sessionExtractor = function (body: Session | SessionPayload | string): Session | null {
    ...
    async (url: string, options?: RequestInit) => {
      ...
      try {
        ...
        if (url.endsWith('/token') || url.includes('/signin/') || url.includes('/signup/')) {
          ...
          // Parse the JSON data
          const body = (await clonedResponse.json().catch(() => null)) as Session | SessionPayload
