...
describe('Enhanced Fetch', () => {
  ...
  test('should apply multiple chain functions in the correct order', async () => {
    ...
    const enhancedFetch = createEnhancedFetch([firstMiddleware, secondMiddleware])
    const url = 'https://api.example.com'
  ...
  test('errors in middleware should propagate', async () => {
    ...
    await expect(enhancedFetch('https://api.example.com')).rejects.toThrow('Middleware error')
    ...
    const errorHandler: ChainFunction =
      ...
      async (url, options = {}) => {
        try {
          ...
          return new Response(JSON.stringify({ error: 'Handled error', fallback: true }), {
            status: 503
          })
        }
      }
  ...
  test('should allow chain functions to modify the URL', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(`${baseUrl}/additional/path`, expect.any(Object))
  })
})
