...
describe('Nhost - Sign Up with Email and Password and upload file', () => {
  ...
  it("errors: no permissions or doesn't exist", async () => {
    try {
      ...
      expect(resp.body.errors).toHaveLength(1)
      const errors = resp.body.errors!
      expect(errors[0]?.message).toBe("field 'restricted' not found in type: 'query_root'")
      expect(error.message).toBe("field 'restricted' not found in type: 'query_root'")
  ...
  it('query with TypedDocumentNode', async () => {
    ...
    const users = await nhost.graphql.request<GetUsersResponse>(GetUsersDocument, {
      limit: 10
    })
