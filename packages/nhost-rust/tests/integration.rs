//! Integration tests against a local Nhost backend (`./dev-env.sh up`). Run
//! them explicitly with `cargo test --test integration -- --include-ignored`.
...
#[tokio::test]
#[ignore = "requires a local backend: ./dev-env.sh up"]
