...
func TestRequestSuccess(t *testing.T) {
	...
	res, _, err := c.Request(context.Background(), "query { __typename }", nil, "", nil)
	if err != nil {
...
func TestRequestGraphQLErrors(t *testing.T) {
	...
	_, _, err := c.Request(context.Background(), "query { nope }", nil, "", nil)
	...
	if apiErr.Error() != "field not found" {
...
func TestExecuteTypedDecode(t *testing.T) {
	...
	res, _, err := graphql.Execute[data](context.Background(), c, "query { user }", nil, "", nil)
	if err != nil {
		t.Fatalf("execute: %v", err)
	}

	if res.Data.User.ID != "u1" || res.Data.User.Name != "Ada" {
		t.Fatalf("decoded = %+v", res.Data.User)
	}
}
...
func TestExecuteNon2xxWithoutGraphQLErrors(t *testing.T) {
	...
	_, _, err := c.Request(context.Background(), "query { me }", nil, "", nil)
