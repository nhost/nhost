...
- `services/engine` - Unified command that runs auth, storage, and constellation behind one shared listener with ordered shutdown
...
- `internal/lib/serve` - shared logger construction, startup flag logging with secret redaction, `Service` handler/background/cleanup abstraction, and ordered multi-service supervision
